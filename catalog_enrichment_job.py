...
def auto_tick(enqueue_limit=20, max_seconds=22.0):
    ...
    # One compact line per cron cycle. Render logs can contain unrelated scanner
    # traffic and many MA-Shops request lines; this marker makes the actual
    # catalogue outcome unambiguous without exposing secrets.
    rr=result.get("run") or {}
    summary={
        "status":rr.get("status"),
        "reason":rr.get("reason"),
        "coin":rr.get("coin"),
        "source_tier":rr.get("source_tier"),
        "canonical":rr.get("canonical"),
        "stored_observations":rr.get("stored_observations"),
        "production_coin_specs_updated":rr.get("production_coin_specs_updated"),
        "query_attempts":rr.get("query_attempts"),
        "elapsed_seconds":result.get("elapsed_seconds"),
    }
    print("[CATALOG AUTO RESULT] "+json.dumps(summary,ensure_ascii=False,default=str),flush=True)
    return result
