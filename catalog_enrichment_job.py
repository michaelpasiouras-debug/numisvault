...
def update_production_coin_specs(coin, official):
    """Write verified issuer fields back to the REAL public.coin_specs row.

    Match is deliberately exact on country + issue year + denomination label
    and, when present, variant. Existing non-null values are preserved unless
    they equal the same official value; this fallback fills missing fields.
    """
    country=str(coin.get("countryEN") or coin.get("country") or "").strip()
    year=int(str(coin.get("year")).strip())
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                where lower(country)=lower(%s)
                  ...
                  and (
                        %s=''
                        or lower(trim(coalesce(variant,'')))=lower(trim(%s))
                      )
                returning id
                """,
                (
                    official.get("primary_metal"),official.get("fineness_per_mille"),
                    official.get("weight_g"),official.get("diameter_mm"),
                    country,year,denom,variant,variant,
                ),
            )
