...
def _coin_specs_candidates(limit=20):
    """Return issue-specific, incomplete records from the REAL production coin_specs table.

    We deliberately do NOT expand year ranges into invented issues.
    Eligible rows must have either issue_year or an exact one-year range.
    """
    limit=max(1,min(int(limit),100))
    with _conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                ...
                where country is not null
                  ...
                  and (
                        primary_metal is null
                        or fineness_per_mille is null
                        or weight_g is null
                        or diameter_mm is null
                      )
                order by
                    case when verified then 1 else 0 end,
                    case when primary_metal is null then 0 else 1 end,
                    case when fineness_per_mille is null then 0 else 1 end,
            ...
            for r in cur.fetchall():
                ...
                # Generic framework/range rows are not issue identities.
                ...
                key=(str(d.get("country") or "").strip().lower(),int(year),
                     round(float(d["denomination_value"]),8),str(d.get("currency_code") or "").lower(),
                     variant.lower())
                if key in seen:
                    continue
                seen.add(key)
...
def preview_production_batch(limit=20):
    ...
    return {
        ...
        "selection_rule":"issue-specific only; incomplete physical specs; no year-range expansion",
