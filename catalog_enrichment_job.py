...
def _coin_specs_candidates(limit=20):
    ...
    for r in rows:
        ...
        # Such rows belong in the integrity audit/quarantine, not the write queue.
...
def run_one(max_seconds=22.0):
    ...
    if "[coin_specs:" in str(job.get("identity_key") or "") and not source_spec_id:
        ...
        _finish_job(
            job["id"],"RETRY",result=result,
            error="SOURCE_SPEC_ID_MISSING",retry_delay_minutes=360
        )
        return {"ok":False,"status":"RETRY",**result}
...
def local_preflight():
    ...
    check("parse_fraction_denom", lambda: (
        (_parse_denom("1/2 Dollar")[0] == 0.5) or
        (_ for _ in ()).throw(AssertionError("fraction denomination failed"))
    ))
