...
def extract_mashops_physical_specs_from_html(html_text, source_url=None):
    """Extract physical specs only from one compact explicit item-spec block.

    Safety rule: never scan the full MA-Shops page for metal/fineness because
    related products can be embedded in the same HTML.
    """
    if not html_text:
        return None
    ...
    label_re=re.compile(
        ...
        r"weight|gewicht|poids|peso|fineness|feinheit|titre|ley)\b",re.I)
    blocks=[]
    for tr in soup.find_all("tr"):
        txt=smart_join(tr.stripped_strings)
        if txt and len(txt)<=260 and label_re.search(txt):
            blocks.append(txt)
    for node in soup.find_all(["p","li","td","span"]):
        if node.find(["p","li","td","tr","div"]):
            continue
        txt=smart_join(node.stripped_strings)
        if txt and len(txt)<=220 and label_re.search(txt):
            blocks.append(txt)
    ...
    for evidence in blocks:
        wm=re.search(
            ...
            r"([0-9]+(?:[.,][0-9]+)?)\s*(?:g|gram|grams|gramm)\b",
            evidence,re.I)
        ...
        cm=re.search(
            ...
            r"(.{1,100}?)(?=\s+(?:weight|gewicht|poids|peso|fineness|feinheit|titre|ley)\b|$)",
            evidence,re.I)
        ...
        fineness=None
        fm=re.search(r"\.\s*([0-9]{3})\b|\b([0-9]{3})\s*‰",comp_text)
        if fm:
            v=int(next(x for x in fm.groups() if x))
            if 100<=v<=999:
                fineness=v
        if fineness is None:
            fm=re.search(
                r"(?:fineness|feinheit|titre|ley)\s*[:\-]?\s*(?:0[.,])?([0-9]{3})"
                r"\s*(?:/\s*1000|‰)?",evidence,re.I)
            if fm:
                v=int(fm.group(1))
                if 100<=v<=999:
                    fineness=v

        # Strong acceptance rule: all three facts must occur together.
        if not (metal and fineness and weight is not None):
            continue
...
def cache_mashops_spec(coin, spec):
    if (spec and spec.get("weight_g") and spec.get("primary_metal")
            and spec.get("fineness_per_mille")):
    ...
    for o in valid[:4]:
        try:
            ...
            if not passes_hard_filter(identity_text,payload):
                print(f"[MA-Shops specs] item identity rejected: {r.url} :: {page_title[:160]}",flush=True)
                continue
            specs=extract_mashops_physical_specs_from_html(r.text,r.url)
            if specs:
                specs.update({
                    "id":None,"title":page_title or o.get("title") or query,"issuer":"","diameter_mm":None,
                    "obverse_image":None,"reverse_image":None,"url":r.url,
                    "match_class":"MA_SHOPS_EXACT_ITEM_SPEC","confidence":0.95,
                    "spec_source_url":r.url,
                    "spec_source_dealer":_dealer_slug_from_offer({"url":r.url}),
                    "spec_source_title":page_title or o.get("title") or query,
                })
...
def verified_spec_override(coin):
    ...
    out.update({
        ...
        "issuer":"","obverse_image":None,"reverse_image":None,"url":None,
...
def coin_lookup():
    ...
    override=verified_spec_override(coin)
    if override:
        print(f"[coin-lookup] provider=verified_override source={override.get('source_reference')} composition={override.get('composition')} weight={override.get('weight_g')}",flush=True)
        return jsonify({"match":override,"provider":"verified_override",
                        "note":"Physical specifications resolved from a verified CoinBids catalogue correction."})
    ...
    if ma_spec:
        print(f"[coin-lookup] provider=ma_shops source={ma_spec.get('spec_source_url')} composition={ma_spec.get('composition')} weight={ma_spec.get('weight_g')}",flush=True)
        return jsonify({"match":ma_spec,"provider":"ma_shops",
                        "note":"Physical specifications extracted from an exact visible MA-Shops item specification."})
    ...
    if not local_match:
        ...
        provider="local"
    if local_match:
        print(f"[coin-lookup] provider={provider} source={local_match.get('spec_source')} composition={local_match.get('composition')} weight={local_match.get('weight_g')}",flush=True)
        return jsonify({"match":local_match,"provider":provider,
                        "note":"Physical specifications resolved from the local catalogue because no exact MA-Shops specification was available."})
