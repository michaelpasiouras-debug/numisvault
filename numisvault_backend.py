...
def enrich_offer_from_item_page(offer):
    ...
    try:
        ...
        # Destination shipping on MA-Shops is often written as
        # "+ 11,50 EUR shipping (to Greece)": amount BEFORE "shipping".
        destination=r"(?:Greece|Griechenland|Gr[eè]ce|Ελλάδα)"
        money_rx=r"((?:EUR\s*|€\s*)?[0-9][0-9., ]*(?:\s*(?:EUR|€))?)"
        ship_words=r"(?:shipping|postage|versand|porto|frais de port|spedizione|envio|envío)"
        patterns=[
            rf"(?:\+\s*)?{money_rx}\s*{ship_words}\s*\(?\s*(?:to\s*)?{destination}\s*\)?",
            rf"{ship_words}\s*\(?\s*(?:to\s*)?{destination}\s*\)?[^€0-9]{{0,30}}{money_rx}",
            rf"{destination}.{{0,100}}?{ship_words}[^€0-9]{{0,30}}{money_rx}",
        ]
        found_shipping=None
        for pattern in patterns:
            ...
            for pat in PRICE_PATTERNS:
                m=pat.search(seg)
                if m:
                    found_shipping=num(m.group(1));break
            if found_shipping is not None: break
        if found_shipping is not None:
            offer["shipping"]=found_shipping
            offer["shipping_status"]="free" if found_shipping==0 else "known"
        else:
            free_to_gr=re.search(
                rf"(?:free shipping|versandkostenfrei|portofrei|livraison gratuite).{{0,100}}?{destination}|"
                rf"{destination}.{{0,100}}?(?:free shipping|versandkostenfrei|portofrei|livraison gratuite)",
                txt,re.I)
            if free_to_gr:
                offer["shipping"]=0.0;offer["shipping_status"]="free"
            else:
                # Never preserve a possibly bogus search-page shipping amount.
                offer["shipping"]=None;offer["shipping_status"]="unknown"
        return offer
    except Exception:
        return offer
