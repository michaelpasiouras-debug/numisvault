...
COUNTRY_CANON = {
    ...
    "san marino":["san marino","san-marino","sanmarino"],
    "vatican city":["vatican city","vatican","citta del vaticano","città del vaticano","vatikan"],
    "slovenia":["slovenia","slovenian","slowenien","slovenie","slovénie"],
    "slovakia":["slovakia","slovak","slowakei","slovaquie"],
    "estonia":["estonia","eesti","estland","estonie"],
    "latvia":["latvia","latvija","lettland","lettonie"],
    "lithuania":["lithuania","lietuva","litauen","lituanie"],
    "cyprus":["cyprus","kypros","zypern","chypre"],
    "malta":["malta","malte"],
    "luxembourg":["luxembourg","luxemburg"],
    "finland":["finland","suomi","finnland","finlande"],
    "ireland":["ireland","eire","éire","irland","irlande"],
}
...
CURRENCY_UNIT_ALIASES = {
    # Modern
    "euro":["euro","euros","eur","european euro"],
    "dollar":["dollar","dollars","usd","united states dollar","us dollar","u.s. dollar","american dollar"],
    "pound":["pound","pounds","gbp","sterling","pound sterling","british pound"],
    ...
    "drachma":["drachma","drachmas","drachmai","drachmae","drachme","drachmen","greek drachma","greek drachmas","grd","δραχμη","δραχμης","δραχμες","δραχμων","δραχμα","δραχμαι","δρχ"],
...
def _canonicalize_denom_text(s):
    """Normalize number words and common fraction forms for denomination parsing.

    Fractions/decimals are normalized BEFORE norm(), because norm() strips '/'
    and '.', which previously corrupted values like 1/2 Dollar.
    """
    raw=ihtml.unescape(str(s or "")).lower().replace(",",".")
    raw=(raw.replace("½","1/2").replace("¼","1/4").replace("¾","3/4")
            .replace("⅒","1/10").replace("⅕","1/5").replace("⅛","1/8"))
    raw=re.sub(r"(?<!\d)1\s*/\s*2(?!\d)","0.5",raw)
    raw=re.sub(r"(?<!\d)1\s*/\s*4(?!\d)","0.25",raw)
    raw=re.sub(r"(?<!\d)3\s*/\s*4(?!\d)","0.75",raw)
    raw=re.sub(r"(?<!\d)1\s*/\s*10(?!\d)","0.1",raw)
    raw=re.sub(r"(?<!\d)1\s*/\s*5(?!\d)","0.2",raw)
    raw=re.sub(r"(?<!\d)1\s*/\s*8(?!\d)","0.125",raw)
    raw=re.sub(r"(?<=\d)\.(?=\d)","zdecimalz",raw)
    a=norm(raw).replace("zdecimalz",".")
...
def variant_tokens(s):
    # Denomination aliases are handled by denomination_matches(). They must not
    # become required variant tokens. Example: MA-Shops often writes
    # "1/2 Dollar 1964 Kennedy" instead of literal "Kennedy Half Dollar".
    stop={
        "coin","commemorative","unc","bu","proof","year",
        "euro","cent","cents","dollar","dollars",
        "half","quarter","one","two","three","four","five","ten"
    }
...
def _resolver_currency_unit(best):
    """Map resolver currency metadata to one of this backend's canonical
    denomination units without maintaining a second currency-code database.

    Prefer a known alias appearing in the resolver's currency name; otherwise
    accept a known currency code alias.  Returns "" if no safe mapping exists.
    """
    if not best:
        return ""
    name=norm(best.get("currency") or "")
    code=norm(best.get("currency_code") or "")
    # longest aliases first avoids "mark" beating "deutsche mark", etc.
    candidates=[]
    for canon,aliases in CURRENCY_UNIT_ALIASES.items():
        for alias in aliases:
            aa=norm(alias)
            if not aa:
                continue
            if aa==code or aa==name or re.search(rf"(?<![a-z0-9]){re.escape(aa)}(?![a-z0-9])",name):
                candidates.append((len(aa),canon))
    return max(candidates)[1] if candidates else ""

def canonicalize_price_search_payload(payload):
    """Language bridge for Price Research.

    The user's raw wording remains available for exact marketplace searching,
    but once the shared resolver has confidently recognized the coin, all hard
    identity checks use one canonical internal country/denomination/year
    representation.  This prevents Greek/French/German/etc. wording from
    changing downstream validation behavior.
    """
    p=dict(payload or {})
    coin=dict(p.get("coin") or {})
    raw=str(p.get("raw_query") or coin.get("raw") or "").strip()
    resolution=None
    if RESOLVER_AVAILABLE and raw:
        try:
            resolution=resolve_coin_identity(raw)
        except Exception as e:
            print(f"[resolver] price-search canonicalization failed for {raw!r}: {type(e).__name__}: {e}",flush=True)
    best=(resolution or {}).get("best")
    if best and (resolution or {}).get("status")=="resolved":
        country=str(best.get("country") or "").strip()
        year=best.get("year")
        value=best.get("denomination_value")
        unit=_resolver_currency_unit(best)
        if country:
            coin["country"]=country
            coin["countryEN"]=country
        if year is not None:
            coin["year"]=str(year)
        if value is not None and unit:
            coin["denom"]=f"{float(value):g} {unit}"
            coin["denomination"]=coin["denom"]
        # Deliberately preserve frontend/local-parser variant/grade hints.
        # They contain meaningful residual text such as "silver eagle".
        coin["raw"]=raw
        p["_resolved_identity"]=best
        p["_resolver_search_variants"]=best.get("search_variants") or []
    p["coin"]=coin
    return p
...
def make_queries(payload):
    ...
    resolver_queries=list(payload.get("_resolver_search_variants") or [])
    if RESOLVER_AVAILABLE and raw and not resolver_queries:
        try:
        ...
        if resolved and resolved.get("best") and resolved.get("status")=="resolved":
            ...
            if not denom and b.get("denomination_value") is not None:
                unit=_resolver_currency_unit(b) or b.get("currency_code") or ""
                denom=f'{b["denomination_value"]:g} {unit}'.strip()
            resolver_queries=b.get("search_variants") or []

    qs = []
    # Search the strongest canonical intent FIRST. This prevents an original
    # non-English/raw spelling from consuming the request budget before the
    # backend reaches the marketplace-friendly canonical query.
    ...
    qs.extend(resolver_queries[:4])
    # Raw wording remains only as a fallback for local names/rare variants.
...
def multilingual_price_research_local_regression():
    """No-network regression matrix for language-neutral identity filtering."""
    checks=[]
    def ck(name,cond):
        if not cond: raise AssertionError(name)
        checks.append({"name":name,"ok":True})

    # Greek raw query resolves to the same canonical 5-drachma hard identity.
    p=canonicalize_price_search_payload({
        "raw_query":"5 δραχμές 1901",
        "coin":{"variant":"","grade":""}
    })
    c=p.get("coin") or {}
    ck("greek_query_country",canonical_country(c.get("country"))=="greece")
    ck("greek_query_denom",parse_denomination(c.get("denom"))==(5.0,"drachma"))
    ck("greek_query_year",str(c.get("year"))=="1901")
    ck("greek_listing_passes",not hard_filter_reasons(
        "Kreta 5 Drachmai 1901 George I VF",p
    ))

    # Same canonical identity from English.
    pe=canonicalize_price_search_payload({
        "raw_query":"5 drachma 1901",
        "coin":{"variant":"","grade":""}
    })
    ck("greek_english_same_identity",
       (pe["coin"].get("country"),pe["coin"].get("denom"),pe["coin"].get("year")) ==
       (c.get("country"),c.get("denom"),c.get("year")))

    # Silver Eagle keeps $1 and explicitly enforces the residual variant.
    ps=canonicalize_price_search_payload({
        "raw_query":"USA 1 Dollar 1987 silver eagle",
        "coin":{"variant":"silver eagle","grade":""}
    })
    ck("silver_eagle_is_one_dollar",parse_denomination(ps["coin"].get("denom"))==(1.0,"dollar"))
    ck("silver_eagle_listing_passes",not hard_filter_reasons(
        "USA American Silver Eagle 1 Dollar 1987 BU",ps
    ))
    ck("constitution_dollar_rejected_for_explicit_variant",
       "VARIANT_MISMATCH" in hard_filter_reasons(
        "USA 1 Dollar 1987 Constitution Bicentennial",ps
    ))

    return {"ok":True,"checks":checks,"database_touched":False,"network_touched":False}


def bounded_price_search_local_regression():
    checks=[]
    def ck(name,cond):
        if not cond: raise AssertionError(name)
        checks.append({"name":name,"ok":True})

    greek=canonicalize_price_search_payload({
        "raw_query":"5 δραχμές 1901",
        "coin":{"variant":"","grade":""}
    })
    gq=make_queries(greek)
    ck("greek_canonical_first", bool(gq) and gq[0].lower()=="greece 5 drachma 1901")
    ck("greek_queries_bounded", len(gq)<=3)

    eagle=canonicalize_price_search_payload({
        "raw_query":"USA 1 Dollar 1987 silver eagle",
        "coin":{"variant":"silver eagle","grade":""}
    })
    eq=make_queries(eagle)
    ck("silver_eagle_variant_first", bool(eq) and "silver eagle" in eq[0].lower())
    ck("silver_eagle_queries_bounded", len(eq)<=3)
    ck("silver_eagle_hard_match",
       not hard_filter_reasons("USA American Silver Eagle 1 Dollar 1987 BU",eagle))
    ck("constitution_rejected",
       "VARIANT_MISMATCH" in hard_filter_reasons(
          "USA 1 Dollar 1987 Constitution Bicentennial",eagle))

    return {"ok":True,"checks":checks,"network_touched":False,"database_touched":False}
...
def _variant_explicit_in_raw(variant, raw_query):
    """True only when the user's own raw text actually states the variant."""
    toks=variant_tokens(variant)
    if not toks:
        return False
    a=norm(raw_query)
    return all(re.search(rf"(?<![a-z0-9]){re.escape(t)}(?![a-z0-9])",a) for t in toks)


def hard_filter_reasons(title, payload):
    """Return explicit reasons a Price Research listing is rejected.

    User-explicit denomination/year are hard requirements.
    Resolver-inferred country/variant may expand search but must not silently
    narrow a raw query that did not state them.
    """
    coin=payload.get("coin") or {}
    ...
    asset,_conf=classify_asset(title)
    if asset=="BANKNOTE":
        reasons.append("BANKNOTE")
    if product_scope(title)!="SINGLE_COIN":
        reasons.append("NOT_SINGLE_COIN")

    raw_denom=parse_denomination(raw_query) if raw_query else None
    structured_denom=str(coin.get("denom") or coin.get("denomination") or "").strip()
    target_denom=(f"{raw_denom[0]:g} {raw_denom[1]}" if raw_denom else structured_denom)
    if target_denom and not denomination_matches(target_denom,title):
        reasons.append("DENOMINATION_MISMATCH")

    structured_year=str(coin.get("year") or "").strip()
    raw_years=re.findall(r"(?<!\d)(1[0-9]{3}|20[0-9]{2}|2100)(?!\d)",raw_query)
    target_year=(raw_years[0] if raw_years else structured_year)
    enforce_year=bool(target_year) and (not raw_query or bool(raw_years))
    if enforce_year and not re.search(rf"(?<!\d){re.escape(target_year)}(?!\d)",a):
        reasons.append("YEAR_MISMATCH")

    country=str(coin.get("country") or coin.get("countryEN") or "").strip()
    enforce_country=bool(country) and (not raw_query or country_in_title(country,raw_query))
    if enforce_country and not country_in_title(country,a):
        reasons.append("COUNTRY_MISMATCH")

    variant=str(coin.get("variant") or "").strip()
    enforce_variant=bool(variant) and (not raw_query or _variant_explicit_in_raw(variant,raw_query))
    if enforce_variant and not variant_matches(variant,title):
        reasons.append("VARIANT_MISMATCH")

    grade=str(coin.get("grade") or "").strip()
    if grade and grade_conflicts(grade,title):
        reasons.append("GRADE_MISMATCH")

    if RESOLVER_AVAILABLE:
        try:
            if get_resolver()._negative_flags(title):
                reasons.append("NEGATIVE_PRODUCT_FLAG")
        except Exception:
            pass
...
def passes_hard_filter(title, payload):
    return not hard_filter_reasons(title,payload)


def price_research_local_regression():
    """DB/network-free checks for the exact cheapest-offer failure class."""
    checks=[]
    ...
    cheap_crete=(
        "Kreta 5 Drachmai 1901 5 Drachmai - Kretischer Staat "
        "1898-1913 VF-,Rdf. / Fassungsspuren"
    )
    payload={
        "raw_query":"5 drachma 1901",
        "coin":{
            ...
            "denom":"5 drachma",
            "year":"1901",
            "variant":"Greek drachma",
        }
    }

    _check("crete_candidate_survives_inferred_greece",
           passes_hard_filter(cheap_crete,payload))
    _check("wrong_year_rejected",
           not passes_hard_filter("Kreta 5 Drachmai 1900 - Kretischer Staat",payload))
    _check("wrong_denomination_rejected",
           not passes_hard_filter("Kreta 2 Drachmai 1901 - Kretischer Staat",payload))

    explicit_greece={
        **payload,
        "raw_query":"Greece 5 drachma 1901",
        "coin":dict(payload["coin"]),
    }
    _check("explicit_greece_is_enforced",
           not passes_hard_filter(cheap_crete,explicit_greece))

    offers=[
        {"price":300.0,"shipping":9.5,"total":309.5},
        {"price":60.0,"shipping":15.0,"total":75.0},
        {"price":145.0,"shipping":9.0,"total":154.0},
        {"price":250.0,"shipping":8.5,"total":258.5},
    ]
    offers.sort(key=lambda o:(
        o.get("total") if o.get("total") is not None else o.get("price",float("inf")),
        o.get("price",float("inf")),
        o.get("total") is None,
    ))
    _check("all_in_sort_returns_75_first",offers[0]["total"]==75.0)
...
def extract_cards(soup, source_url, payload):
    ...
    for iid,g in groups.items():
        ...
        # MA-Shops search rows can explicitly state destination shipping, e.g.
        # "+ 15,00 EUR shipping (to Greece)".  When the destination printed in
        # the SAME listing row matches the user's requested ship_to country,
        # that shipping amount is valid ranking evidence and must NOT be thrown
        # away.  The previous code discarded it as "unverified", which caused
        # cheap offers (60/145/250 EUR) to receive total=None and be sorted
        # behind a 300 EUR offer whose seller happened to exist in the local
        # shipping DB.
        requested_ship_to=str(payload.get("ship_to") or "").strip()
        if shipping is not None and shipping_status!="free":
            row_low=norm(row_text)
            dest_tokens=[]
            if requested_ship_to:
                dest_tokens.append(norm(requested_ship_to))
                if norm(requested_ship_to)=="greece":
                    dest_tokens.extend(["griechenland","grece","hellas","ellada","ελλαδα","ελλάδα"])
            destination_explicit=bool(dest_tokens and any(tok and tok in row_low for tok in dest_tokens))
            if destination_explicit:
                shipping_status="known_target_search_row"
            else:
...
def extract_mashops_explicit_specs(html_text, source_url=None):
    ...
    """
    # IMPORTANT: multi-word alloy names must be matched BEFORE bare metal words.
    # "Nordic gold" is a copper alloy; treating the token "gold" inside it as
    # precious Gold corrupts euro-cent catalogue records and Metal Value.
    metal_aliases=[
        ("nordic gold","Nordic gold"),("nordisches gold","Nordic gold"),
        ("or nordique","Nordic gold"),("oro nordico","Nordic gold"),
        ("oro nórdico","Nordic gold"),("oro nordic","Nordic gold"),
    ...
    for evidence in _mashops_spec_blocks(html_text):
        ...
        if comp:
            ...
            for raw,canon in metal_aliases:
...
_EURO_SPEC_COUNTRIES = {
    ...
    "slovakia","slovenia","spain","vatican city","vatican"
}

def _catalogue_spec_denom(country, denom):
    """Translate legacy unitless euro catalogue values only for euro-profile rows.

    public.coin_specs contains legacy denominations such as 0.5, 0.2, 0.1 etc.
    Dealer titles use 50 Cent, 20 Cent, 10 Cent.  Price Research user input is
    untouched; this normalization is ONLY for catalogue physical-spec evidence.
    """
    raw=str(denom or "").strip().replace(",",".")
    if parse_denomination(raw):
        return raw
    if canonical_country(country) not in _EURO_SPEC_COUNTRIES:
        return raw
    try:
        v=float(raw)
    except Exception:
    ...
    if v < 1:
        ...
        if abs(cents-round(cents))<1e-9:
            return f"{int(round(cents))} cent"
    if abs(v-round(v))<1e-9:
        return f"{int(round(v))} euro"
    return f"{v:g} euro"


def _catalogue_spec_candidate_reasons(text, payload):
    """Hard identity checks for catalogue evidence, separate from Price Research."""
    coin=payload.get("coin") or {}
    reasons=[]
    if classify_asset(text)[0]=="BANKNOTE": reasons.append("BANKNOTE")
    if product_scope(text)!="SINGLE_COIN": reasons.append("NOT_SINGLE_COIN")
    denom=str(coin.get("denom") or "").strip()
    if denom and not denomination_matches(denom,text): reasons.append("DENOMINATION_MISMATCH")
    year=str(coin.get("year") or "").strip()
    if year and not re.search(rf"(?<!\\d){re.escape(year)}(?!\\d)",norm(text)):
        reasons.append("YEAR_MISMATCH")
    country=str(coin.get("country") or coin.get("countryEN") or "").strip()
    if country and not country_in_title(country,text): reasons.append("COUNTRY_MISMATCH")
    return reasons


def catalogue_spec_local_regression():
    checks=[]
    def ck(name,cond):
        if not cond: raise AssertionError(name)
        checks.append({"name":name,"ok":True})
    ck("greece_0_5_normalizes_to_50_cent",_catalogue_spec_denom("Greece","0.5")=="50 cent")
    p={"coin":{"country":"Greece","countryEN":"Greece","denom":"50 cent","year":"1999"}}
    ck("localized_greece_50_cent_passes",not _catalogue_spec_candidate_reasons("Griechenland 50 Cent 1999",p))
    ck("wrong_year_rejected","YEAR_MISMATCH" in _catalogue_spec_candidate_reasons("Griechenland 50 Cent 2000",p))
    ck("wrong_denom_rejected","DENOMINATION_MISMATCH" in _catalogue_spec_candidate_reasons("Griechenland 20 Cent 1999",p))
    return {"ok":True,"checks":checks,"database_touched":False,"network_touched":False}

_EURO_CENT_PHYSICAL_PROFILES = {
    "10 cent": (4.10, 19.75, "Nordic gold"),
    "20 cent": (5.74, 22.25, "Nordic gold"),
    "50 cent": (7.80, 24.25, "Nordic gold"),
}

def _guard_euro_cent_spec(coin, accepted):
    """Fail closed on impossible precious-metal classification of standard euro cents."""
    country=canonical_country(str(coin.get("countryEN") or coin.get("country") or ""))
    if country not in _EURO_SPEC_COUNTRIES:
        return accepted, None
    denom=_catalogue_spec_denom(country, coin.get("denom"))
    profile=_EURO_CENT_PHYSICAL_PROFILES.get(str(denom).strip().lower())
    if not profile:
        return accepted, None
    ew,ed,expected=profile
    metal=str(accepted.get("primary_metal") or "").strip().lower()
    w=accepted.get("weight_g"); d=accepted.get("diameter_mm")
    physical_match=(w is not None and d is not None and abs(float(w)-ew)<=0.08 and abs(float(d)-ed)<=0.18)
    if physical_match and metal in {"gold","silver","platinum","palladium","electrum"}:
        # Never silently rewrite marketplace evidence. Suppress the contradictory
        # metal/composition/fineness while retaining independently-consensed dimensions.
        accepted=dict(accepted)
        accepted.pop("primary_metal",None); accepted.pop("composition",None)
        accepted.pop("fineness_per_mille",None); accepted.pop("fine_metal_g",None)
        return accepted, {"code":"EURO_CENT_PRECIOUS_METAL_CONFLICT","expected":expected,"observed":metal,"weight_g":w,"diameter_mm":d}
    return accepted, None
...
def mashops_spec_fallback(coin, raw_query=""):
    ...
    spec_denom=_catalogue_spec_denom(country,denom)
    payload={"coin":{"country":country,"countryEN":country,"denom":spec_denom,"year":year,"variant":variant},
    ...
    for o in offers:
        ...
        reasons=_catalogue_spec_candidate_reasons(mt,payload)
        if reasons:
            for reason in reasons:
                rejection_counts[reason]=rejection_counts.get(reason,0)+1
            if len(rejection_samples)<12:
                rejection_samples.append({"title":o.get("title"),"price":o.get("price"),"reasons":reasons})
            continue
        valid.append(o)
    valid.sort(key=lambda o:(o.get("price") is None,o.get("price") or float("inf")))

    evidence=[];checked=0
    print(f"[MA-Shops specs] candidates offers={len(offers)} valid={len(valid)} query={query!r} normalized_denom={spec_denom!r} rejected={rejection_counts}",flush=True)
    if not valid and rejection_samples:
        print(f"[MA-Shops specs] rejection_samples={rejection_samples}",flush=True)
    for o in valid[:20]:
        try:
            ...
            page_reasons=_catalogue_spec_candidate_reasons(identity_text,payload)
            if page_reasons:
                print(f"[MA-Shops specs] page reject title={page_title!r} reasons={page_reasons}",flush=True)
                continue
    ...
    accepted,euro_guard=_guard_euro_cent_spec(coin,accepted)
    if euro_guard:
        print(f"[MA-Shops specs] SAFETY_GUARD {euro_guard}",flush=True)
...
def inspect_mashops_item_asset(url, timeout=4):
...
def _ma_search_page_links(soup, current_url, query):
    """Discover MA-Shops pagination links from the actual response markup.

    We deliberately do NOT guess a page parameter.  MA-Shops has changed its
    search URL shape over time; following only links emitted by the current
    result page is safer and keeps Price Research compatible with either form.
    """
    out=[]
    qnorm=norm(query)
    cur=urllib.parse.urlsplit(current_url)
    for a in soup.find_all("a", href=True):
        href=urllib.parse.urljoin(current_url,a.get("href"))
        try:
            u=urllib.parse.urlsplit(href)
        except Exception:
            continue
        if u.netloc and u.netloc.lower()!=cur.netloc.lower():
            continue
        if not u.path.endswith("/shops/search.php") and not u.path.endswith("search.php"):
            continue
        qs=urllib.parse.parse_qs(u.query)
        # A pagination link must remain on the same search.  Accept searchstr
        # either literally or URL-decoded; ignore category/navigation searches.
        sq=(qs.get("searchstr") or [""])[0]
        if sq and norm(sq)!=qnorm:
            continue
        # Require a pagination-looking signal: numeric link text OR a query
        # parameter other than searchstr/sortby/language/currency.
        txt=" ".join(a.stripped_strings).strip().lower()
        extra=set(qs)-{"searchstr","sortby","lang","language","curr","currency"}
        looks_page=bool(re.fullmatch(r"\d+",txt) or txt in {"next",">",">>","›","»","weiter"} or extra)
        if not looks_page:
            continue
        # Force cheapest-first even if MA-Shops' emitted pager omits sortby.
        pairs=urllib.parse.parse_qsl(u.query,keep_blank_values=True)
        d=dict(pairs);d["searchstr"]=query;d["sortby"]="preis_eur"
        clean=urllib.parse.urlunsplit((u.scheme or cur.scheme,u.netloc or cur.netloc,u.path,u.query,""))
        clean=urllib.parse.urlunsplit((u.scheme or cur.scheme,u.netloc or cur.netloc,u.path,urllib.parse.urlencode(d),""))
        if clean!=current_url and clean not in out:
            out.append(clean)
    return out


def fetch_search(query, payload):
    """Crawl MA-Shops cheapest-first search results for one query.

    Critical Price Research invariant: never call the first parsed page "the
    cheapest offers" when MA-Shops exposes additional result pages.  We follow
    pagination links emitted by MA-Shops itself and keep crawling until either
    enough *identity-valid* candidates have been seen or the bounded crawl
    budget is exhausted.  Final identity/detail validation still happens in
    coin_search().
    """
    last_err=None
    combined=[]
    used_urls=[]
    seen_urls=set()
    queue=list(ma_urls(query))
    started=time.monotonic()
    MAX_PAGES=2
    MAX_SECONDS=8.0
    MIN_VALID_CANDIDATES=6
    valid_seen=0

    while queue and len(seen_urls)<MAX_PAGES and (time.monotonic()-started)<MAX_SECONDS:
        url=queue.pop(0)
        if url in seen_urls:continue
        seen_urls.add(url)
        try:
            remaining=max(3.0,MAX_SECONDS-(time.monotonic()-started))
            r=SESSION.get(url,timeout=min(6.0,remaining),allow_redirects=True)
            ...
            if "captcha" in r.text.lower() and len(r.text)<200000:
                ...
                print("[MA-Shops]   -> looks like a CAPTCHA/anti-bot page", flush=True)
                continue
            soup=BeautifulSoup(r.text,"html.parser")
            offers=extract_from_jsonld(soup,r.url,payload)
            offers+=extract_cards(soup,r.url,payload)
            # De-duplicate within this page before counting identity-valid rows.
            page_by={}
            for o in offers:
                o["dealer_source"]="MA-Shops"
                k=o.get("url") or (o.get("title"),o.get("price"))
                if k not in page_by or o.get("_score",0)>page_by[k].get("_score",0):
                    page_by[k]=o
            offers=list(page_by.values())
            print(f"[MA-Shops]   -> parsed {len(offers)} unique offer(s)", flush=True)
            if offers:
                combined.extend(offers)
                used_urls.append(r.url)
                for o in offers:
                    mt=o.get("_match_text") or o.get("title","")
                    if passes_hard_filter(mt,payload):
                        valid_seen+=1
            else:
                last_err="No offer blocks parsed"

            # Discover real pagination from the returned document.  Because the
            # result is cheapest-first, earlier pages are always crawled first.
            for nxt in _ma_search_page_links(soup,r.url,query):
                if nxt not in seen_urls and nxt not in queue:
                    queue.append(nxt)

            # Ten row-valid candidates is deliberately wider than the displayed
            # top-2, leaving room for detail-page banknote/identity rejection.
            if valid_seen>=MIN_VALID_CANDIDATES:
                break
        except Exception as e:
            last_err=str(e)
    ...
    if combined:
        print(f"[MA-Shops] crawl query={query!r} pages={len(used_urls)} row_valid={valid_seen} offers={len(combined)}",flush=True)
...
# Earliest national euro-coin year accepted by CoinBids catalogue matching.
# This is a safety guard against legacy broad EUR rows that started all countries
# at 1999. It does not alter the database; the worker has a one-time repair routine.
...
_EURO_NUMERIC_DENOMS={0.01,0.02,0.05,0.10,0.20,0.50,1.00,2.00}

def _euro_year_is_impossible(country, year, denom_value):
    """True only for an exact standard euro denomination before that issuer's
    earliest accepted euro coin year. This is deliberately narrow."""
    try:
        c=canonical_country(country)
        y=int(year)
        d=round(float(denom_value),2)
    except Exception:
        return False
    start=_EURO_COIN_START_YEAR.get(c)
    return bool(start and d in _EURO_NUMERIC_DENOMS and y < start)
...
def pg_coin_spec_match(coin):
    ...
    if year is not None and _euro_year_is_impossible(country,year,denom):
        print(f"[coin-specs-pg] blocked impossible euro year: {country} {denom:g} {year}", flush=True)
        return None
...
def local_coin_spec_match(coin):
    ...
    if not country or denom is None:return None
    if year is not None and _euro_year_is_impossible(country,year,denom):
        print(f"[coin-specs-json] blocked impossible euro year: {country} {denom:g} {year}", flush=True)
        return None
...
def coin_lookup():
    ...
    # Fast UI mode for Identify Coin / Auction Intelligence: use verified local
    # PostgreSQL/JSON catalogue data first and return immediately. This keeps
    # specification display off the critical path and prevents a hidden
    # marketplace crawl from monopolizing a single synchronous web worker.
    if fast:
        ...
        if local_match:
            return jsonify({"match":local_match,"provider":provider,
                            "note":"Fast verified local catalogue lookup."})
        return jsonify({"match":None,"provider":"local",
                        "note":"No verified local specification match. Use Price Research/catalogue enrichment to expand coverage."})
...
def _search_cache_key(payload):
    coin=payload.get("coin") or {}
    cache_schema="price-research-delivered-price-v8"
...
def coin_search():
    payload=request.get_json(silent=True) or {}
    payload=canonicalize_price_search_payload(payload)
    ...
    MAX_CONCURRENT_MA_SHOPS_REQUESTS=2
    ...
    try:
        ...
        # bounded search window and ignore/cancel laggards. The window is 11s; each query is independently capped at two cheapest-first pages and eight seconds.
        done,not_done=wait(set(future_to_query.keys()),timeout=11)
        for future in done:
    ...
    cheap_rejections=[]
    explicit_denom_rejection_logs=0
    valid=[]
    offers_for_validation=sorted(
        offers,
        key=lambda o:(
            o.get("price") is None,
    ...
    for o in offers_for_validation:
        ...
        if _target_denom and _explicit_title_denoms and denomination_conflicts(_target_denom,o.get("title","")):
            if explicit_denom_rejection_logs<20:
                ...
                explicit_denom_rejection_logs+=1
            rejected["identity"]+=1;continue
        _hard_reasons=hard_filter_reasons(match_text,payload)
        if _hard_reasons:
            rejected["identity"]+=1
            if len(cheap_rejections)<20:
                cheap_rejections.append({
                    "title":o.get("title"),
                    "price":o.get("price"),
                ...
                print(
                    f"[coin-search] reject candidate price={o.get('price')} "
                    f"title={o.get('title')!r} reasons={_hard_reasons}",
                    flush=True
                )
            continue
        if target_identity:
    ...
    _diag_coin=payload.get("coin") or {}
    _diag_country=str(_diag_coin.get("country") or "").strip()
    _diag_raw=str(payload.get("raw_query") or "").strip()
    _diag_country_enforced=bool(_diag_country) and (not _diag_raw or country_in_title(_diag_country,_diag_raw))
    print(f"[coin-search] raw-intent country={_diag_country!r} enforced={_diag_country_enforced} raw={_diag_raw!r}",flush=True)
    ...
    for o in valid:
        finalize(o)
        if include_shipping:
            # Prefer shipping explicitly printed by MA-Shops for the requested
            # destination in this exact result row.  It is direct evidence and
            # is more appropriate for purchase ranking than discarding the
            # cheapest offer merely because our local seller DB has no tier.
            if o.get("shipping_status")=="known_target_search_row" and o.get("shipping") is not None:
                o["shipping_source"]="ma_shops_search_row_target"
                finalize(o)
                ...
                # Unknown shipping must never turn a cheap VALID item into an
                # apparently more expensive item. Preserve item price and mark
                # total unknown; final ranking below uses item price as the
                # lower-bound fallback instead of pushing it behind expensive
                # offers with known shipping.
                o["shipping"]=None
    ...
    #
    # IMPORTANT: when shipping is included, an offer with UNKNOWN shipping has
    # only a lower bound (item price), not a comparable delivered total. It must
    # never outrank an offer whose delivered total is actually known.
    include_shipping=bool(payload.get("include_shipping",True))
    if include_shipping:
        valid.sort(
            key=lambda o:(
                o.get("total") is None,  # known delivered totals FIRST
    ...
    # Explicit summaries for the UI. "best_offer" means cheapest comparable
    # delivered price when shipping is included; an unknown-shipping listing
    # is exposed separately as the lowest item-price lower bound.
    known_delivered=[o for o in valid if o.get("total") is not None]
    cheapest_known_delivered=(min(known_delivered,key=lambda o:o.get("total"))
                              if known_delivered else None)
    priced=[o for o in valid if o.get("price") is not None]
    lowest_item_price=(min(priced,key=lambda o:o.get("price")) if priced else None)
    ...
    cheapest_known_delivered_public=(public_offer(cheapest_known_delivered)
                                     if cheapest_known_delivered else None)
    lowest_item_price_public=(public_offer(lowest_item_price)
                              if lowest_item_price else None)
    ...
    diagnostics={
        ...
        "raw_intent_contract":{
            "raw_query":str(payload.get("raw_query") or "").strip(),
            "resolver_country_hard_filter_enforced":_diag_country_enforced,
            "rule":"resolver-inferred country/variant expand search but do not silently narrow raw user intent",
        },
    }
    ...
    result={
        ...
        "best_offer":(
            cheapest_known_delivered_public if include_shipping and cheapest_known_delivered_public
            else (top_public[0] if top_public else None)
        ),
        "cheapest_known_delivered":cheapest_known_delivered_public,
        "lowest_item_price":lowest_item_price_public,
        "count":len(top_public),"raw_count":raw_count,
        ...
        "note":(
            "No exact validated MA-Shops match was found. Candidate listings may have been returned but rejected for explicit year, denomination, country, variant, banknote/set, or negative-product mismatches; see diagnostics.cheap_rejections."
            if not top_public else
            "When shipping is included, offers with a confirmed delivered total rank before offers whose shipping is unknown. Unknown shipping is never treated as zero/free. The lowest raw item price is reported separately when useful."
        ),
...
# ============================================================
# CATALOGUE ENRICHMENT ADMIN ENDPOINTS
# One queued coin per request; designed for Render Free + cron.
# ============================================================

CATALOG_ADMIN_KEY = os.environ.get("CATALOG_ADMIN_KEY", "").strip()

def _catalog_admin_authorized():
    supplied=(request.args.get("key") or request.headers.get("X-Catalog-Admin-Key") or "").strip()
    return bool(CATALOG_ADMIN_KEY and supplied and secrets.compare_digest(supplied,CATALOG_ADMIN_KEY))

@app.get("/api/admin/catalog/status")
def catalog_admin_status():
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        import catalog_enrichment_job as cj
        with _CATALOG_AUTO_TICK_STATE_LOCK:
            auto_state=dict(_CATALOG_AUTO_TICK_STATE)
        # Never expose credentials. State contains only execution/result data.
        return jsonify({"ok":True,"queue":cj.queue_status(),"auto_tick":auto_state})
    except Exception as e:
        app.logger.exception("catalog status failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500


@app.get("/api/admin/catalog/preview-production-batch")
def catalog_admin_preview_production_batch():
    """READ ONLY preview from the actual production coin_specs table."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        ...
        try:
            ...
            return jsonify({"ok":False,"error":"limit_must_be_integer_1_to_20"}),400
        import catalog_enrichment_job as cj
        return jsonify(cj.preview_production_batch(limit=limit))
    except Exception as e:
        app.logger.exception("catalog preview production batch failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500


@app.get("/api/admin/catalog/enqueue-production-batch")
def catalog_admin_enqueue_production_batch():
    """Enqueue up to 20 real issue-specific records from production coin_specs."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        limit_raw=(request.args.get("limit") or "20").strip()
        try:
            ...
            return jsonify({"ok":False,"error":"limit_must_be_integer_1_to_20"}),400
        import catalog_enrichment_job as cj
        result=cj.enqueue_production_batch(limit=limit,priority_start=1000)
        result["queue"]=cj.queue_status()
        return jsonify(result)
    except Exception as e:
        app.logger.exception("catalog enqueue production batch failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500



@app.get("/api/admin/catalog/repair-official")
def catalog_admin_repair_official():
    """One-time/idempotent reseed of curated Tier-1 issuer observations."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        import catalog_enrichment_job as cj
        result=cj.repair_official_registry()
        result["queue"]=cj.queue_status()
        return jsonify(result)
    except Exception as e:
        app.logger.exception("catalog official repair failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500



@app.get("/api/admin/catalog/audit")
def catalog_admin_audit():
    """Read-only audit of public.coin_specs completeness and selector coverage."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    ...
    try:
        ...
        if not db_url:
            return jsonify({"ok":False,"error":"DATABASE_URL_missing"}),500

        conn=psycopg2.connect(db_url)
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                # Column discovery makes the audit tolerant of optional fields.
                cur.execute("""
                    select column_name
                    from information_schema.columns
                    where table_schema='public' and table_name='coin_specs'
                """)
                cols={r["column_name"] for r in cur.fetchall()}

                required={"id","country","denomination_label","weight_g","diameter_mm","primary_metal"}
                missing=sorted(required-cols)
                if missing:
                    return jsonify({
                        "ok":False,
                        "read_only":True,
                        "error":"coin_specs_schema_missing_required_columns",
                        "missing_columns":missing,
                        "available_columns":sorted(cols),
                    }),500

                year_expr = (
                    "issue_year" if "issue_year" in cols
                    else ("year_from" if "year_from" in cols else "NULL")
                )
                year_from_expr = "year_from" if "year_from" in cols else year_expr
                year_to_expr = "year_to" if "year_to" in cols else year_expr
                fineness_expr = "fineness_per_mille" if "fineness_per_mille" in cols else "NULL"
                variant_expr = "variant" if "variant" in cols else "NULL"
                verified_expr = "verified" if "verified" in cols else "NULL"
                confidence_expr = "confidence" if "confidence" in cols else "NULL"

                # Snapshot rows once; all classification below is read-only Python logic.
                cur.execute(f"""
                    select
                        id,
                        country,
                        ...
                        {year_expr} as issue_year,
                        {year_from_expr} as year_from,
                        {year_to_expr} as year_to,
                        weight_g,
                        diameter_mm,
                        primary_metal,
                        {fineness_expr} as fineness_per_mille,
                        {verified_expr} as verified,
                        {confidence_expr} as confidence
                    from public.coin_specs
                    order by country, coalesce({year_expr},{year_from_expr}), denomination_label, id
                """)
        ...
        def is_precious_or_fineness_relevant(metal):
            m=norm(metal)
            # Fineness is expected for precious-metal catalogue records, not ordinary base alloys.
            return any(token in m for token in (
                "silver","gold","platinum","palladium","electrum"
            ))

        def issue_specific(r):
            iy=r.get("issue_year")
            yf=r.get("year_from")
            yt=r.get("year_to")
            if iy is not None:
                return True
            return yf is not None and yt is not None and str(yf)==str(yt)

        counts={
            "total":len(rows),
            "issue_specific":0,
            "year_range_or_unspecified":0,
            "complete_metal_aware":0,
            "incomplete_metal_aware":0,
            "missing_weight":0,
            "missing_diameter":0,
            "missing_primary_metal":0,
            "missing_fineness_precious_only":0,
            "base_metal_without_fineness_but_complete":0,
        }
        samples={
            "missing_weight":[],
            "missing_diameter":[],
            "missing_primary_metal":[],
            "missing_fineness_precious_only":[],
            "year_range_or_unspecified":[],
        }
        ...
        def sample_row(r):
            return {
                ...
                "year":r.get("issue_year") if r.get("issue_year") is not None else r.get("year_from"),
                "year_to":r.get("year_to"),
                "denom":r.get("denomination_label"),
        ...
        for r in rows:
            if not issue_specific(r):
                counts["year_range_or_unspecified"]+=1
                if len(samples["year_range_or_unspecified"])<20:
                    samples["year_range_or_unspecified"].append(sample_row(r))
                continue

            counts["issue_specific"]+=1
            missing=[]

            if r.get("weight_g") is None:
                counts["missing_weight"]+=1
                missing.append("weight_g")
                if len(samples["missing_weight"])<20:
                    samples["missing_weight"].append(sample_row(r))

            if r.get("diameter_mm") is None:
                counts["missing_diameter"]+=1
                missing.append("diameter_mm")
                if len(samples["missing_diameter"])<20:
                    samples["missing_diameter"].append(sample_row(r))

            metal=r.get("primary_metal")
            if metal is None or not norm(metal):
                counts["missing_primary_metal"]+=1
                missing.append("primary_metal")
                if len(samples["missing_primary_metal"])<20:
                    samples["missing_primary_metal"].append(sample_row(r))
            elif is_precious_or_fineness_relevant(metal) and r.get("fineness_per_mille") is None:
                counts["missing_fineness_precious_only"]+=1
                missing.append("fineness_per_mille")
                if len(samples["missing_fineness_precious_only"])<20:
                    samples["missing_fineness_precious_only"].append(sample_row(r))
            elif not is_precious_or_fineness_relevant(metal) and r.get("fineness_per_mille") is None:
                if r.get("weight_g") is not None and r.get("diameter_mm") is not None:
                    counts["base_metal_without_fineness_but_complete"]+=1

            if missing:
                counts["incomplete_metal_aware"]+=1
            else:
                counts["complete_metal_aware"]+=1

        # Ask the actual production selector too, without writing anything.
        selector_preview=[]
        selector_error=None
        try:
            import catalog_enrichment_job as cj
            selector_preview=cj._coin_specs_candidates(100)
        except Exception as e:
            selector_error={"type":type(e).__name__,"message":str(e)[:500]}
        ...
        return jsonify({
            ...
            "counts":counts,
            "selector":{
                "candidate_count_returned":len(selector_preview),
                "preview":selector_preview[:20],
                "error":selector_error,
            },
            "samples":samples,
            "note":"No catalogue, queue, or observation rows were modified.",
        })
    except Exception as e:
        app.logger.exception("catalog audit failed")
        return jsonify({
            "ok":False,
            "read_only":True,
...
@app.get("/api/admin/catalog/repair-nordic-gold")
# Schema-safe repair implementation lives in catalog_enrichment_job.py
def catalog_admin_repair_nordic_gold():
    """One-time conservative repair for the MA-Shops Nordic-gold parser bug."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    if str(request.args.get("confirm") or "").upper()!="YES":
        return jsonify({"ok":False,"confirmation_required":True,
                        "message":"Re-run with confirm=YES after deploying the Nordic-gold fix."}),400
    try:
        import catalog_enrichment_job as cj
        return jsonify(cj.repair_false_gold_euro_cents())
    except Exception as e:
        app.logger.exception("Nordic gold repair failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500


@app.get("/api/admin/catalog/integrity-audit")
def catalog_admin_integrity_audit():
    """Read-only contradiction/quality audit before catalogue automation."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        import psycopg2
        ...
        db_url=(os.environ.get("DATABASE_URL") or "").strip()
        if not db_url:
            return jsonify({"ok":False,"read_only":True,"error":"DATABASE_URL_missing"}),500

        with psycopg2.connect(db_url) as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    select id,country,denomination_label,variant,issue_year,year_from,year_to,
                           weight_g,diameter_mm,primary_metal,fineness_per_mille,
                           verified,confidence
                    from public.coin_specs
                    order by country,coalesce(issue_year,year_from),denomination_label,id
                """)
                rows=[dict(r) for r in cur.fetchall()]

        def outrow(r, reason=None):
            return {
                "id":str(r.get("id")),
                "country":r.get("country"),
                "denom":r.get("denomination_label"),
        ...
        for r in rows:
            conflict=cj._euro_profile_conflict(r)
            if conflict:
                euro_conflicts.append(outrow(r,conflict))
            ...
            if fin is not None:
                try:
                    fv=float(fin)
                    if fv<=0 or fv>1000:
                        impossible_fineness.append(outrow(r,{
                            "code":"FINENESS_OUT_OF_RANGE","value":fv
                        }))
                except Exception:
                    impossible_fineness.append(outrow(r,{
                        "code":"FINENESS_NOT_NUMERIC","value":str(fin)
                    }))

            if r.get("issue_year") is None and r.get("year_from") is None:
                missing_anchor.append(outrow(r,{"code":"NO_YEAR_ANCHOR"}))

            # Fingerprint only complete physical dimensions; cross-country repeats
            # are informational because standard circulation systems can share specs.
            if r.get("weight_g") is not None and r.get("diameter_mm") is not None:
                fp=(
                    round(float(r["weight_g"]),3),
                    round(float(r["diameter_mm"]),3),
                    str(r.get("primary_metal") or "").strip().lower(),
                )
                duplicate_fingerprints.setdefault(fp,[]).append(r)

        repeated=[]
        for fp,group in duplicate_fingerprints.items():
            countries=sorted({str(x.get("country") or "") for x in group})
            if len(group)>=4 and len(countries)>=3:
                repeated.append({
                    "fingerprint":{
                        "weight_g":fp[0],"diameter_mm":fp[1],"primary_metal":fp[2] or None
                    },
                    "row_count":len(group),
                    "country_count":len(countries),
                    "countries":countries[:20],
                    "sample_ids":[str(x.get("id")) for x in group[:20]],
                    "classification":"INFORMATIONAL_REPEAT_NOT_AUTO_ERROR",
                })
        repeated.sort(key=lambda x:(-x["row_count"],-x["country_count"]))

        selector=cj._coin_specs_candidates(500)
        selector_ids={str(x.get("source_spec_id")) for x in selector}
        quarantined_ids={str(x["id"]) for x in euro_conflicts}
        overlap=sorted(selector_ids & quarantined_ids)

        preflight=cj.local_preflight()

        blocking = bool(euro_conflicts or impossible_fineness or missing_anchor or overlap or not preflight.get("ok"))

        return jsonify({
            "ok":True,
            "read_only":True,
            "automation_safe_to_enable":not blocking,
            "blocking_issue_count":(
                len(euro_conflicts)+len(impossible_fineness)+len(missing_anchor)+len(overlap)
                +(0 if preflight.get("ok") else 1)
            ),
            "counts":{
                "total_rows":len(rows),
                "selector_candidates_after_quarantine":len(selector),
                "euro_profile_metal_conflicts":len(euro_conflicts),
                "impossible_fineness":len(impossible_fineness),
                "missing_year_anchor":len(missing_anchor),
                "quarantine_selector_overlap":len(overlap),
                "repeated_physical_fingerprints_informational":len(repeated),
            },
            "blocking":{
                "euro_profile_metal_conflicts":euro_conflicts[:100],
                "impossible_fineness":impossible_fineness[:100],
                "missing_year_anchor":missing_anchor[:100],
                "quarantine_selector_overlap_ids":overlap[:100],
            },
            "informational":{
                "repeated_physical_fingerprints":repeated[:50]
            },
            "preflight":preflight,
            "note":"Read-only. No catalogue, queue, observation, or coin_specs rows were modified."
        })
    except Exception as e:
        app.logger.exception("catalog integrity audit failed")
        return jsonify({
...
# The external cron service has a hard 30-second HTTP timeout. Catalogue
# enrichment may legitimately take longer than that because it performs
# network lookups. Do NOT keep the cron HTTP request open while that work runs.
# Start one in-process background cycle and acknowledge the cron immediately.
_CATALOG_AUTO_TICK_LOCK = threading.Lock()
_CATALOG_AUTO_TICK_STATE_LOCK = threading.Lock()
_CATALOG_AUTO_TICK_STATE = {
    "running": False,
    "started_at": None,
    "finished_at": None,
    "last_result": None,
...
def _catalog_auto_tick_background(limit):
    # Lock is acquired synchronously by the request handler before this thread
    # starts, closing the small race where two cron requests could both start.
    try:
        with _CATALOG_AUTO_TICK_STATE_LOCK:
            _CATALOG_AUTO_TICK_STATE.update(
                running=True, started_at=time.time(), finished_at=None,
                last_error=None
            )
        try:
            app.logger.info("[CATALOG AUTO START] limit=%s",limit)
            import catalog_enrichment_job as cj
            result=cj.auto_tick(enqueue_limit=limit,max_seconds=22.0)
            rr=(result or {}).get("run") or {}
            app.logger.info(
                "[CATALOG AUTO END] status=%s reason=%s coin=%s source=%s elapsed=%s",
                rr.get("status"),rr.get("reason"),rr.get("coin"),
                rr.get("source_tier"),(result or {}).get("elapsed_seconds")
            )
            with _CATALOG_AUTO_TICK_STATE_LOCK:
                _CATALOG_AUTO_TICK_STATE["last_result"]=result
        except Exception as e:
            app.logger.exception("catalog auto-tick background cycle failed")
            with _CATALOG_AUTO_TICK_STATE_LOCK:
                _CATALOG_AUTO_TICK_STATE["last_error"]={
                    "error":type(e).__name__, "message":str(e)[:500]
                }
        finally:
            with _CATALOG_AUTO_TICK_STATE_LOCK:
                _CATALOG_AUTO_TICK_STATE["running"]=False
                _CATALOG_AUTO_TICK_STATE["finished_at"]=time.time()
    finally:
        _CATALOG_AUTO_TICK_LOCK.release()

@app.get("/api/admin/catalog/auto-tick")
def catalog_admin_auto_tick():
    """Cron-safe trigger: acknowledge immediately; run one cycle in background."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    ...
    # Acquire BEFORE spawning the worker. This makes the overlap guard atomic
    # even if two cron requests arrive in the same millisecond.
    if not _CATALOG_AUTO_TICK_LOCK.acquire(blocking=False):
        with _CATALOG_AUTO_TICK_STATE_LOCK:
            state=dict(_CATALOG_AUTO_TICK_STATE)
        return jsonify({
            "ok":True, "accepted":False, "status":"ALREADY_RUNNING",
            "message":"Previous catalogue auto-tick is still running.",
            "state":state,
        }),200

    worker=threading.Thread(
        target=_catalog_auto_tick_background, args=(limit,),
        name="catalog-auto-tick", daemon=True
    )
    try:
        worker.start()
    except Exception:
        ...
        raise
    return jsonify({
        "ok":True, "accepted":True, "status":"STARTED",
        "message":"Catalogue auto-tick started in background.",
        "limit":limit,
    }),202



@app.get("/api/admin/catalog/euro-year-audit")
def catalog_admin_euro_year_audit():
    """Read-only audit of impossible legacy standard-euro year anchors."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        import catalog_enrichment_job as cj
        rows=[]
        with cj._conn() as conn:
            with conn.cursor(cursor_factory=cj.RealDictCursor) as cur:
                cur.execute("""
                    select id,country,denomination_label,issue_year,year_from,year_to,
                           weight_g,diameter_mm,primary_metal
                    from public.coin_specs order by country,id
                """)
                for r in cur.fetchall():
                    d=dict(r)
                    reason=cj._invalid_euro_year_reason(d)
                    if reason:
                        rows.append({"id":str(d["id"]),**reason})
        return jsonify({"ok":True,"read_only":True,"count":len(rows),"rows":rows})
    except Exception as e:
        app.logger.exception("euro year audit failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500


@app.get("/api/admin/catalog/repair-euro-years")
def catalog_admin_repair_euro_years():
    """Idempotent one-time repair of legacy standard-euro series year_from values."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    confirm=(request.args.get("confirm") or "").strip().upper()
    if confirm!="YES":
        return jsonify({
            "ok":False,"error":"confirmation_required",
            "message":"Re-run with confirm=YES after checking /euro-year-audit."
        }),400
    try:
        import catalog_enrichment_job as cj
        return jsonify(cj.repair_euro_year_applicability())
    except Exception as e:
        app.logger.exception("euro year repair failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500


@app.get("/api/admin/catalog/enqueue-test")
def catalog_admin_enqueue_test():
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        import catalog_enrichment_job as cj
        ids=cj.enqueue_test_batch()
        return jsonify({"ok":True,"enqueued":len(ids),"job_ids":ids,"queue":cj.queue_status()})
    except Exception as e:
        app.logger.exception("catalog enqueue test failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500


@app.get("/api/admin/catalog/jobs")
def catalog_admin_jobs():
    """Read-only inspection of catalogue enrichment jobs."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        ...
        try:
            ...
            return jsonify({"ok":False,"error":"limit_must_be_integer_1_to_100"}),400

        import catalog_enrichment_job as cj
        with cj._conn() as conn:
            with conn.cursor(cursor_factory=cj.psycopg2.extras.RealDictCursor) as cur:
                cur.execute("""
                    select q.id,q.coin_id,q.identity_key,q.country,q.year,
                           q.denomination_text,q.variant,q.priority,q.status,
                           q.attempts,q.last_error,q.result,q.next_run_at,
                           q.started_at,q.finished_at,q.created_at,q.updated_at,
                           c.primary_metal,c.composition,c.fineness_per_mille,
                           c.weight_g,c.diameter_mm,c.fine_metal_g,
                           c.specs_confidence,c.observation_count,
                           c.verified_observation_count
                    from public.catalog_enrichment_queue q
                    left join public.coin_catalog c on c.id=q.coin_id
                    order by
                        case q.status
                            when 'RUNNING' then 0
                            when 'RETRY' then 1
        ...
        return jsonify({"ok":True,"read_only":True,"count":len(jobs),
                        "summary":summary,"jobs":jobs})
    except Exception as e:
        app.logger.exception("catalog jobs inspection failed")
        return jsonify({"ok":False,"error":type(e).__name__,
                        "message":str(e)[:500]}),500



@app.get("/api/admin/catalog/retry-now")
def catalog_admin_retry_now():
    """Move exactly one RETRY/FAILED catalogue job back to PENDING now."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    ...
    try:
        ...
        with cj._conn() as conn:
            with conn.cursor(cursor_factory=cj.psycopg2.extras.RealDictCursor) as cur:
                cur.execute(
                    """
                    select id,status,country,year,denomination_text,variant,attempts
                    from public.catalog_enrichment_queue
                    where id=%s
                    for update
                    """,
                ...
                cur.execute(
                    ...
                    set status='PENDING',
                        ...
                        updated_at=now()
                    where id=%s
                    returning id,status,country,year,denomination_text,variant,attempts,next_run_at
                    """,
                    (job_id,),
                )
        ...
        return jsonify({
            "ok":True,
            "message":"Exactly one catalogue job moved to PENDING for immediate retry.",
            "job":updated
        })
    except Exception as e:
        app.logger.exception("catalog retry-now failed")
        return jsonify({
            "ok":False,
            "error":type(e).__name__,
            "message":str(e)[:500]
...
@app.get("/api/admin/catalog/run-one")
def catalog_admin_run_one():
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        import catalog_enrichment_job as cj
        result=cj.run_one(max_seconds=22.0)
        return jsonify(result)
    except Exception as e:
        app.logger.exception("catalog run-one failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500
