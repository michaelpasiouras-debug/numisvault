...
def enrich_offer_from_item_page(offer):
    ...
    This reports FOUR distinct confidence levels instead of collapsing
    everything into unknown/known:
    ...
    end up textually adjacent to this item's "shipping"/"Greece" mention
    ...
    try:
        ...
        if not segments:
            segments=[" ".join(soup.stripped_strings)]
        ...
        gr_patterns=[
            ...
            rf"{ship_words}\s*\(?\s*(?:to\s*)?{destination_gr}\s*\)?[^€0-9]{{0,20}}{money_rx}",
            rf"{destination_gr}[^€0-9]{{0,40}}?{ship_words}[^€0-9]{{0,20}}{money_rx}",
        ]
        rx=search_segments(gr_patterns)
        if rx:
        ...
        free_patterns=[
            rf"(?:free shipping|versandkostenfrei|portofrei|livraison gratuite)[^€0-9]{{0,40}}?{destination_gr}",
            rf"{destination_gr}[^€0-9]{{0,40}}?(?:free shipping|versandkostenfrei|portofrei|livraison gratuite)",
        ]
