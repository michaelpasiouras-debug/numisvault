...
def _search_cache_key(payload):
    coin=payload.get("coin") or {}
    cache_schema="price-research-bounded-canonical-v7"
...
def coin_search():
    ...
    valid.sort(
        key=lambda o:(
            # Rank by confirmed all-in total when known; otherwise by the item
            # price lower bound. Crucially, unknown shipping no longer sends a
            # 60 EUR valid offer behind a 300 EUR offer.
            (o.get("total") if o.get("total") is not None else
             (o.get("price") if o.get("price") is not None else float("inf"))),
            o.get("price") if o.get("price") is not None else float("inf"),
            o.get("total") is None,
