...
def make_queries(payload):
    ...
    qs = []
    # Canonical resolver variants first: after identity resolution, marketplace
    # discovery should be language-neutral. Keep the literal raw wording as a
    # safety-net query for unusual local names/variants.
...
def fetch_search(query, payload):
    ...
    while queue and len(seen_urls)<MAX_PAGES and (time.monotonic()-started)<MAX_SECONDS:
        ...
        try:
            ...
            r=SESSION.get(url,timeout=min(10.0,remaining),allow_redirects=True)
...
def _search_cache_key(payload):
    coin=payload.get("coin") or {}
    cache_schema="price-research-multilingual-canonical-v5"
...
def coin_search():
    ...
    try:
        ...
        # bounded search window and ignore/cancel laggards. The window is 24s because each query may now follow real MA-Shops pagination to prove the cheapest valid candidates.
        done,not_done=wait(set(future_to_query.keys()),timeout=24)
        for future in done:
