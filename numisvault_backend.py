...
def strict_listing_title_denomination_matches(title, payload):
    """Title-level denomination guard shared by Price Research and Specs."""
    coin=payload.get("coin") or {}
    target_text=str(coin.get("denom") or coin.get("denomination") or "").strip()
    target=parse_denomination(target_text)
    if not target:
        return True
    stated=extract_denominations(title or "")
    if not stated:
        return True
    tv,tu=target
    return any(abs(float(v)-float(tv))<1e-9 and u==tu for v,u in stated)


def shared_offer_identity_valid(offer, payload, text=None):
    """Single identity contract used by both price and specification pipelines."""
    title=str((offer or {}).get("title") or "").strip()
    match_text=str(text if text is not None else ((offer or {}).get("_match_text") or title))
    if classify_asset(match_text)[0]=="BANKNOTE":
        return False
    if product_scope(match_text)!="SINGLE_COIN":
        return False
    if not passes_hard_filter(match_text,payload):
        return False
    if title and not strict_listing_title_denomination_matches(title,payload):
        return False
    return True
...
def mashops_spec_fallback(coin, raw_query=""):
    ...
    for o in offers:
        ...
        if not shared_offer_identity_valid(o,payload,mt):
            continue
        valid.append(o)
    ...
    for o in valid[:20]:
        try:
            ...
            if page_title and not strict_listing_title_denomination_matches(page_title,payload):
                print(f"[MA-Shops specs] reject page-title denomination: {page_title!r}",flush=True)
                continue
...
def validate_ambiguous_mashops_assets(offers, payload, max_checks=18):
    ...
    for o in ranked:
        ...
        # Every candidate in this bounded cheapest window gets actual page validation.
        ...
        if page_title and (
            not passes_hard_filter(page_title,payload)
            or not strict_listing_title_denomination_matches(page_title,payload)
        ):
            o["_identity_page_validation"]="REJECT"
            o["_identity_page_reject_title"]=page_title
            rejected+=1
            print(f"[coin-search] reject detail identity: {page_title!r}",flush=True)
            continue
        if page_title:
...
def metal_spot():
    """Return current precious-metal spot prices in USD/troy oz plus USD->EUR.

    Primary feed: gold-api.com real-time endpoints (XAU/XAG/XPT/XPD).
    Fallback for XAU/XAG: goldprice.org. Results are indicative spot prices,
    cached by the browser; they are not numismatic market values.
    """
    symbols={"gold":"XAU","silver":"XAG","platinum":"XPT","palladium":"XPD"}
    prices={}; errors=[]; sources=[]
    for name,symbol in symbols.items():
        try:
            r=SESSION.get(f"https://api.gold-api.com/price/{symbol}",timeout=10)
            r.raise_for_status()
            d=r.json()
            val=d.get("price")
            if val is None:
                val=d.get("ask") or d.get("bid")
            if val is None or float(val)<=0:
                raise ValueError(f"unexpected {symbol} response")
            prices[name]=float(val)
            if "gold-api.com" not in sources:sources.append("gold-api.com")
        except Exception as e:
            errors.append(f"{symbol}: {type(e).__name__}: {e}")

    # Independent fallback for the two metals most commonly found in coins.
    if "gold" not in prices or "silver" not in prices:
        try:
            r=SESSION.get("https://data-asg.goldprice.org/dbXRates/USD",timeout=12)
            r.raise_for_status(); d=r.json(); item=(d.get("items") or [None])[0]
            if item:
                ...
                if "goldprice.org" not in sources:sources.append("goldprice.org")
        except Exception as e:
            errors.append(f"fallback: {type(e).__name__}: {e}")

    try:
        rates=fx_rates(); eur=rates.get("EUR")
        if not eur: raise ValueError("EUR exchange rate unavailable")
    except Exception as e:
        print(f"[metal-spot] FX {type(e).__name__}: {e}",flush=True)
        return jsonify({"error":"EUR exchange rate unavailable"}),503

    if not prices:
        print("[metal-spot] no prices: "+" | ".join(errors),flush=True)
        return jsonify({"error":"live metal price unavailable","details":errors}),503

    out={"usd_to_eur":float(eur),"source":" + ".join(sources),"unit":"USD/troy oz","troy_ounce_g":31.1034768}
    for name,val in prices.items(): out[f"{name}_usd_oz"]=val
...
def coin_search():
    ...
    try:
        for future in as_completed(future_to_query):
            ...
            # IMPORTANT: collect every bounded query result.  Stopping after the
            # first completed query returned >=80 offers made the result depend on
            # thread timing: a broad query could finish first and prevent the exact
            # raw/variant query (e.g. "usa 1 dollar silber 1987") from ever entering
            # the candidate pool.
    ...
    for o in offers:
        ...
        _real_title=str(o.get("title") or "").strip()
        if _real_title and not strict_listing_title_denomination_matches(_real_title,payload):
            rejected["identity"]+=1
            o["_identity_reject_reason"]="title_denomination_conflict"
            print(f"[coin-search] reject title denomination: {_real_title!r}",flush=True)
            continue
        if target_identity:
            try:
    ...
    valid,asset_detail_checked,asset_detail_rejected=validate_ambiguous_mashops_assets(valid,payload,max_checks=12)
    ...
    print("[coin-search] cheapest-valid="+str([
        {"title":x.get("title"),"price":x.get("price"),"shipping":x.get("shipping"),
         ...
         "denoms":extract_denominations(x.get("title") or "")}
        for x in valid[:5]
    ]),flush=True)
