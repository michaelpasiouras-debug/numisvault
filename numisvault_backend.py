...
def enrich_offer_from_item_page(offer, ship_to_country="Greece"):
    ...
    try:
        # Best-effort attempt to nudge the page toward a destination-specific
        # shipping display. NONE of this is verified to actually work — I
        # cannot browse the live MA-Shops site from here to confirm it, and
        # I'm not going to pretend otherwise. If MA-Shops determines the
        # shown destination purely by request IP geolocation or a JS-driven
        # calculator (both plausible), none of these signals will change
        # anything, and the existing DOM-scoped extraction below simply
        # reports whatever destination actually appears on the page — same
        # honest fallback behavior as before, no regression either way.
        dest_hint_headers={"Accept-Language":f"{dest_iso.lower()},en;q=0.5"} if dest_iso else {}
        parsed=urlsplit(url)
        ...
        if dest_iso:
            for param in ("country","land","ship_to","ship_country","dest","destination"):
                if param not in qs:
                    new_qs=dict(qs);new_qs[param]=dest_iso
                    candidate_urls.append(urlunsplit(parsed._replace(query=urlencode(new_qs))))
                    break  # try one plausible variant, not a combinatorial pile of requests
        r=None
        for cand in candidate_urls:
            try:
                resp=SESSION.get(cand,timeout=12,allow_redirects=True,headers=dest_hint_headers)
                if resp.ok:
                    r=resp
                    if re.search(dest_pattern,resp.text,re.I):
                        break  # this variant actually shows the target destination — stop here
            except Exception:
                continue
...
def coin_search():
    ...
    for o in to_enrich.values():enrich_offer_from_item_page(o,ship_to_country)
