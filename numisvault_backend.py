...
def _canonicalize_denom_text(s):
    ...
    IMPORTANT: convert slash fractions BEFORE norm().  norm() intentionally strips
    punctuation (including '/'), so the old order silently turned ``1/2 Dollar``
    into ``1 2 dollar`` and parse_denomination() then read it as **2 Dollar**.
    That made genuine Kennedy Half Dollar listings fail the hard denomination gate.
    """
    raw=ihtml.unescape(str(s or "")).lower().replace(",",".")
    # Convert common numismatic slash fractions while the slash still exists.
