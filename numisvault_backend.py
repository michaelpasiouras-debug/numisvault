...
# ============================================================
# TEMPORARY CATALOGUE CONSENSUS DIAGNOSTIC V2 — DRY RUN ONLY
# Added 2026-08-22.
#
# IMPORTANT:
# - No INSERT / UPDATE / DELETE is performed.
# - Existing Price Research / MA-Shops / shipping / metal value
#   routes and logic are not modified.
# - The existing mashops_spec_fallback() is used only to gather
#   identity-validated evidence.
# - This layer applies stricter CANONICAL-CATALOGUE rules.
# ============================================================
...
def _catalog_unique_dealer_rows(rows):
    """One observation per dealer; keeps catalogue consensus source-independent."""
    out=[]
    seen=set()
    for row in rows or []:
...
def _catalog_exact_text_consensus(rows, min_dealers=2):
    ...
    if len(grp)<min_dealers:
        return None,grp,len(grp)
    return grp[0].get("value"),grp,len(grp)

def _catalog_exact_numeric_consensus(rows, decimals=2, min_dealers=2):
    """Canonical numeric value requires repeated rounded value across dealers.

    This deliberately does NOT publish the median of different physical
    specimens as the official mint/catalogue specification.
    """
    rows=_catalog_unique_dealer_rows(rows)
    groups={}
    for row in rows:
...
def _catalogue_consensus_from_mashops_spec(spec):
    """Convert live-market observations into conservative canonical candidates.

    Rules:
    * metal: exact agreement from >=2 independent dealers
    * fineness: exact agreement from >=2 independent dealers
    * canonical weight: same value to 0.01 g from >=2 independent dealers
      (different worn/specimen weights are retained only as observations)
    * diameter: same value to 0.10 mm from >=2 independent dealers
    * fine metal content is calculated ONLY from canonical weight + fineness
    """
    if not spec:
        return {
            "catalogue_status":"NO_CONSENSUS",
            "metal_specs_status":"UNVERIFIED",
            "dimensions_status":"UNVERIFIED",
            "canonical":{},
            "observed":{},
            "field_confidence":{},
            "notes":["MA-Shops did not produce accepted evidence for this identity."],
        }
    ...
    metal,metal_grp,metal_n=_catalog_exact_text_consensus(metal_rows,2)
    fine,fine_grp,fine_n=_catalog_exact_numeric_consensus(fine_rows,0,2)
    weight,weight_grp,weight_n=_catalog_exact_numeric_consensus(weight_rows,2,2)
    diam,diam_grp,diam_n=_catalog_exact_numeric_consensus(diam_rows,1,2)

    canonical={}
    if metal is not None:
        canonical["primary_metal"]=metal
    if fine is not None:
        canonical["fineness_per_mille"]=int(round(float(fine)))
    if weight is not None:
        canonical["weight_g"]=round(float(weight),2)
    if diam is not None:
        canonical["diameter_mm"]=round(float(diam),1)
    ...
    # Never compute melt weight from a median specimen measurement.
    ...
    field_confidence={
        "primary_metal":{
            "status":"VERIFIED" if metal is not None else "UNVERIFIED",
            "agreeing_dealers":metal_n,
            "required_dealers":2,
        },
        "fineness_per_mille":{
            "status":"VERIFIED" if fine is not None else "UNVERIFIED",
            "agreeing_dealers":fine_n,
            "required_dealers":2,
        },
        "weight_g":{
            "status":"VERIFIED_CANONICAL_CANDIDATE" if weight is not None else "OBSERVATIONS_ONLY",
            "agreeing_dealers":weight_n,
            "required_dealers":2,
            "rule":"same rounded value to 0.01 g across independent dealers",
        },
        "diameter_mm":{
            "status":"VERIFIED_CANONICAL_CANDIDATE" if diam is not None else "OBSERVATIONS_ONLY",
            "agreeing_dealers":diam_n,
            "required_dealers":2,
            "rule":"same rounded value to 0.10 mm across independent dealers",
        },
    }

    metal_specs_ok=all(k in canonical for k in ("primary_metal","fineness_per_mille","weight_g"))
    dimensions_ok="diameter_mm" in canonical

    if metal_specs_ok and dimensions_ok:
        catalogue_status="VERIFIED_COMPLETE"
    elif metal_specs_ok:
        catalogue_status="VERIFIED_METAL_SPECS"
    elif canonical.get("primary_metal") and canonical.get("fineness_per_mille") is not None:
        catalogue_status="PARTIAL_NEEDS_CANONICAL_WEIGHT"
    else:
        catalogue_status="PARTIAL_UNVERIFIED"

    notes=[]
    if weight is None and weight_rows:
        notes.append(
            "Dealer weights are preserved as specimen observations; no repeated 0.01 g value exists, "
            "so no official/canonical weight is published."
        )
    if diam is None and diam_rows:
        notes.append(
            "Diameter evidence is insufficient for a canonical diameter (requires >=2 independent dealers agreeing to 0.10 mm)."
        )
    if not diam_rows:
        notes.append("No usable diameter observations were found.")
    if not metal_specs_ok:
        notes.append("fine_metal_g is intentionally withheld until canonical weight and fineness are verified.")

    return {
        "catalogue_status":catalogue_status,
        "metal_specs_status":"VERIFIED" if metal_specs_ok else "UNVERIFIED",
        "dimensions_status":"VERIFIED" if dimensions_ok else "UNVERIFIED",
        "canonical":canonical,
        "observed":observed,
        ...
        "source":{
            "provider":"MA-Shops",
            "raw_consensus_confidence":spec.get("confidence"),
...
def catalog_builder_test():
    if not CATALOG_DIAGNOSTIC_KEY:
        return jsonify({
            "ok":False,
            "error":"catalog_diagnostic_disabled",
            "message":"Set CATALOG_DIAGNOSTIC_KEY in Render."
        }),503
    ...
    try:
        idx=int(request.args.get("coin","0"))
    except Exception:
    ...
    if idx<0 or idx>=len(_CATALOG_DIAGNOSTIC_TEST_COINS):
        return jsonify({
            "ok":False,
            "error":"coin_out_of_range",
            "valid_coin_indexes":list(range(len(_CATALOG_DIAGNOSTIC_TEST_COINS)))
        }),400
    ...
    try:
        ...
        catalogue=_catalogue_consensus_from_mashops_spec(raw_spec)
        return jsonify({
            "ok":True,
            "mode":"DRY_RUN_V2",
            ...
            "catalogue":catalogue,
            "message":"Catalogue consensus evaluated; database unchanged.",
        })
    except Exception as e:
        app.logger.exception("catalog-builder diagnostic V2 failed")
        return jsonify({
            "ok":False,
            "mode":"DRY_RUN_V2",
            "database_writes":False,
