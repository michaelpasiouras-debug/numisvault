...
def make_queries(payload):
    ...
    if RESOLVER_AVAILABLE and raw and not (country and denom and year):
        ...
        if resolved and resolved.get("best") and resolved.get("status") in ("resolved","review") and not resolved.get("ambiguous"):
            b=resolved["best"]
...
@app.get("/health")
def health():
    return jsonify({"ok":True,"service":"CoinBids backend (MA-Shops + Numista)","numista_configured":bool(NUMISTA_API_KEY),"resolver_available":RESOLVER_AVAILABLE})
