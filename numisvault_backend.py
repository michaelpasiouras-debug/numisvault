...
@app.get("/api/admin/catalog/preview-real-batch")
def catalog_admin_preview_real_batch():
    """READ ONLY: preview up to 20 real catalogue rows eligible for bulk enqueue."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        ...
        import catalog_enrichment_job as cj
        test_keys = {
            cj._identity_key("United States", 1922, 1, "dollar", "Peace Dollar"),
            cj._identity_key("United States", 1923, 1, "dollar", "Peace Dollar"),
            cj._identity_key("United States", 1924, 1, "dollar", "Peace Dollar"),
            cj._identity_key("United States", 1925, 1, "dollar", "Peace Dollar"),
            cj._identity_key("United States", 1964, 0.5, "dollar", "Kennedy Half Dollar"),
        }
        with cj._conn() as conn:
            with conn.cursor(cursor_factory=cj.psycopg2.extras.RealDictCursor) as cur:
                cur.execute(
                    """
                    select c.id,c.identity_key,c.country,c.year,c.denomination,
                           c.currency,c.variant,c.specs_confidence,c.primary_metal,
                           c.fineness_per_mille,c.weight_g,c.diameter_mm
                    from public.coin_catalog c
                    where not exists (
                        select 1 from public.catalog_enrichment_queue q
                ...
                for r in cur.fetchall():
                    d=dict(r)
                    if str(d.get("identity_key") or "") in test_keys:
                        continue
                    rows.append({
                        "coin_id":str(d["id"]),
                        ...
                        "year":d.get("year"),
                        "denomination":float(d["denomination"]) if d.get("denomination") is not None else None,
                        "currency":d.get("currency"),
                        "variant":d.get("variant"),
                        "specs_confidence":d.get("specs_confidence"),
        ...
        return jsonify({
            ...
            "count":len(rows),
            "coins":rows,
            "message":"Preview only; no catalogue or queue rows were changed."
        })
    except Exception as e:
        app.logger.exception("catalog preview real batch failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500


@app.get("/api/admin/catalog/enqueue-real-batch")
def catalog_admin_enqueue_real_batch():
    """Enqueue up to 20 real catalogue coins after preview."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        limit_raw=(request.args.get("limit") or "20").strip()
        try:
            ...
            return jsonify({"ok":False,"error":"limit_must_be_integer_1_to_20"}),400
        import catalog_enrichment_job as cj
        result=cj.enqueue_real_batch(limit=limit,priority_start=1000)
        result["queue"]=cj.queue_status()
        return jsonify(result)
    except Exception as e:
        app.logger.exception("catalog enqueue real batch failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500
