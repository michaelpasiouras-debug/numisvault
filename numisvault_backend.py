...
def public_identify_coin():
    # Standalone Identify Coin was retired; Price Research owns the shared
    # free-text identity-resolution flow. Keep old bookmarks useful.
    return redirect("/coin-value", code=301)
...
def coin_search():
    ...
    result={
        ...
        "note":"When shipping is included, offers with a confirmed delivered total rank before offers whose shipping is unknown. Unknown shipping is never treated as zero/free. The lowest raw item price is reported separately when useful.",
