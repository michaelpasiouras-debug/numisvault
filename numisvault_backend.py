...
def extract_mashops_partial_specs_from_html(html_text, source_url=None):
    """Extract MA-Shops physical specs from ONE coherent specification cluster.

    Critical safety change:
    - do not harvest isolated "Weight"/"Silver"/".900" tokens page-wide;
    - do not combine facts from related/recommended products embedded in HTML;
    - require a compact block containing at least THREE distinct physical-spec
      labels among Material/Composition, Weight, Fineness, Diameter;
    - use only the best/earliest coherent cluster on the main item page.

    This matches the visible MA-Shops item layout such as:
      Material: Silver
      Weight: 26.73 g
      Fineness: 900 ‰
      Diameter: 37.00 mm
    ...
    # Cut obvious recommendation/footer areas out of consideration.
    stop_re=re.compile(
        r"(?:recently viewed|similar items|you may also like|recommend(?:ed|ations?)|"
        r"weitere artikel|ähnliche artikel|customer support|newsletter)",
        re.I)

    def label_flags(txt):
        return {
            "material": bool(re.search(r"\b(?:composition|material|metal|legierung|materiale|m[ée]tal)\b",txt,re.I)),
            "weight": bool(re.search(r"\b(?:weight|gewicht|poids|peso)\b",txt,re.I)),
            "fineness": bool(re.search(r"\b(?:fineness|feinheit|titre|ley)\b",txt,re.I)),
            "diameter": bool(re.search(r"\b(?:diameter|durchmesser|diam[eè]tre|diametro)\b|[Øø]",txt,re.I)),
        }

    # Candidate containers: compact DOM sections, never the whole page/body.
    candidates=[]
    order=0
    for node in soup.find_all(["div","td","section","table","p","li"]):
        order+=1
        txt=smart_join(node.stripped_strings)
        ...
        if stop_re.search(txt):
            continue
        flags=label_flags(txt)
        nlabels=sum(flags.values())
        if nlabels < 3:
            continue
        # Avoid containers that clearly aggregate multiple products.
        years=set(re.findall(r"\b(?:1[5-9]\d{2}|20\d{2})\b",txt))
        # Multiple years are not automatically wrong (ruler periods can appear),
        # but 3+ distinct years in a tiny block strongly signals related items.
        if len(years)>=3:
            continue
        candidates.append((nlabels, -len(txt), -order, txt))

    if not candidates:
        return {}

    # Prefer most complete, then most compact, then earliest.
    ...
    cm=re.search(
        ...
        r"diameter|durchmesser|diam[eè]tre|diametro|catalog|katalog)\b|$)",
        evidence,re.I)
    ...
    fin=None
    fm=re.search(r"(?:fineness|feinheit|titre|ley)\s*[:\-]?\s*([0-9]{3})\s*‰",evidence,re.I)
    ...
    if fin is None:
        ...
        if fm: fin=int(fm.group(1))
    if fin is None and comp_text:
        fm=re.search(r"(?<!\d)\.\s*([0-9]{3})\b|0[.,]([0-9]{3})\b",comp_text)
        if fm: fin=int(next(x for x in fm.groups() if x))
    if fin is not None and 100<=fin<=999:
        out["fineness_per_mille"]={"value":fin,"evidence":evidence}

    # Final coherence gate: after parsing, at least 3 physical facts must
    # actually have been recovered from this SAME cluster.
    if len(out) < 3:
        return {}
    return out
    ...
    for o in valid[:max_item_pages]:
        try:
            ...
            if not passes_hard_filter(identity_text,payload):
                print(f"[MA-Shops consensus] identity rejected hard-filter: {r.url} :: {page_title[:140]}",flush=True)
                continue
            identity_score=score_title(identity_text,payload)
            # Specification consensus is stricter than price discovery.
            # A listing can be good enough to show as a market offer but still
            # be too weak to become catalogue/spec evidence.
            if identity_score < 0.72:
                print(f"[MA-Shops consensus] identity rejected low-score={identity_score:.3f}: {r.url} :: {page_title[:140]}",flush=True)
                continue
            ...
            for field,item in partial.items():
                ...
                ev[field].append({
                    ...
                    "title":page_title,"identity_score":identity_score,
    ...
    for field,grp in groups.items():
        evidence_by_field[field]=[
            {"dealer":x.get("dealer"),"title":x.get("title"),"url":x.get("url"),
             "identity_score":x.get("identity_score"),
