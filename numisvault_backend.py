...
DENOM_UNIT_ALT = (r"euro\s*cent(?:s)?|eurocent(?:s)?|"
                   r"euro(?:s)?|eur|€|cent(?:s)?|¢|dollar(?:s)?|usd|\$|pound(?:s)?|gbp|£|franc(?:s)?|"
                   r"kuna|lipa|drachma(?:s)?|ruble(?:s)?|rouble(?:s)?|kopeck(?:s)?|yen|yuan|zloty|"
                   r"forint|koruna|krona|leu|lev|dinar(?:s)?")
...
def _normalize_denom_unit(unit):
    ...
    if u in ("euro cent","eurocent","euro cents","eurocents"):return "cent"
    if u in ("eur","€","euros"):return "euro"
    if u in ("cents","¢"):return "cent"
    if u in ("usd","$","dollars"):return "dollar"
    if u in ("gbp","£","pounds"):return "pound"
    if u=="francs":return "franc"
    return u
...
def enrich_offer_from_item_page(offer):
    ...
    — not Greece specifically, unless the seller happens to be Greek or the
    page does geo-detection we can't replicate with a plain GET request.
    Treating "no literal 'to Greece' text" as "shipping unknown" was
    technically honest but threw away real, visible information the person
    could see themselves by opening the same link. This now reports THREE
    distinct confidence levels instead of collapsing them all into
    unknown/known:
      known_greece              - literally confirmed "to Greece"
      ...
      known_unconfirmed_destination - a flat shipping figure was found with
                                   no destination text at all (common for
                                   sellers who only ship at one flat rate)
    ...
    try:
        ...
        destination_gr=r"(?:Greece|Griechenland|Gr[eè]ce|Ελλάδα)"
        money_rx=r"((?:EUR\s*|€\s*)?[0-9][0-9., ]*(?:\s*(?:EUR|€))?)"
        ...
        # 1) Greece-specific, amount-before-label and label-before-amount orderings.
        gr_patterns=[
            rf"(?:\+\s*)?{money_rx}\s*{ship_words}\s*\(?\s*(?:to\s*)?{destination_gr}\s*\)?",
            rf"{ship_words}\s*\(?\s*(?:to\s*)?{destination_gr}\s*\)?[^€0-9]{{0,30}}{money_rx}",
            rf"{destination_gr}.{{0,100}}?{ship_words}[^€0-9]{{0,30}}{money_rx}",
        ]
        found_shipping=None
        for pattern in gr_patterns:
            rx=re.search(pattern,txt,re.I)
            if rx:
                found_shipping=find_money(rx.group(1))
                if found_shipping is not None: break
        if found_shipping is not None:
            offer["shipping"]=found_shipping
            offer["shipping_status"]="free" if found_shipping==0 else "known_greece"
            offer["shipping_destination"]="Greece"
            return offer

        # 2) A specific OTHER destination was found (e.g. "to Germany") —
        # real information, just not for Greece. Preserve the destination name.
        other_pattern=rf"(?:\+\s*)?{money_rx}\s*{ship_words}\s*\(?\s*(?:to|nach|vers|a)\s+{generic_dest}\s*\)?"
        rx=re.search(other_pattern,txt,re.I)
        if rx:
            amt=find_money(rx.group(1));dest=rx.group(2)
            if amt is not None and dest and dest.lower() not in ("greece","griechenland","grece","gr\u00e8ce"):
                offer["shipping"]=amt
                offer["shipping_status"]="known_unconfirmed_destination" if amt==0 else "known_other_destination"
                offer["shipping_destination"]=dest.strip()
                return offer

        # 3) A flat shipping figure with no destination text at all.
        bare_pattern=rf"(?:\+\s*)?{money_rx}\s*{ship_words}(?!\s*\()"
        rx=re.search(bare_pattern,txt,re.I)
        if rx:
        ...
        # 4) General free-shipping claim (with or without a Greece mention).
        free_to_gr=re.search(
            rf"(?:free shipping|versandkostenfrei|portofrei|livraison gratuite).{{0,100}}?{destination_gr}|"
            rf"{destination_gr}.{{0,100}}?(?:free shipping|versandkostenfrei|portofrei|livraison gratuite)",
            txt,re.I)
        if free_to_gr:
            offer["shipping"]=0.0;offer["shipping_status"]="free";offer["shipping_destination"]="Greece"
            return offer
        if re.search(r"(?:free shipping|versandkostenfrei|portofrei|livraison gratuite)",txt,re.I):
...
def coin_search():
    ...
    for o in to_enrich.values():enrich_offer_from_item_page(o)
    include_shipping=bool(payload.get("include_shipping"))
    target_currency=(payload.get("currency") or "EUR").upper()
    for o in valid:
        # A shipping figure that never got item-page verification must not
        # silently feed the all-in total — treat it as unknown for totals.
