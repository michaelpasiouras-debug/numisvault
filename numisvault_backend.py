...
def catalog_admin_auto_tick():
    """Bounded cron-safe cycle: enqueue new candidates + process one due job."""
    if not _catalog_admin_authorized():
        return jsonify({"ok":False,"error":"unauthorized"}),401
    try:
        limit_raw=(request.args.get("limit") or "20").strip()
        try:
            ...
            return jsonify({"ok":False,"error":"limit_must_be_integer_1_to_20"}),400
        import catalog_enrichment_job as cj
        return jsonify(cj.auto_tick(enqueue_limit=limit,max_seconds=22.0))
    except Exception as e:
        app.logger.exception("catalog auto-tick failed")
        return jsonify({"ok":False,"error":type(e).__name__,"message":str(e)[:500]}),500
