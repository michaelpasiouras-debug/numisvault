...
# ============================================================
# TEMPORARY CATALOG BUILDER DIAGNOSTIC — DRY RUN ONLY
# ============================================================
...
def _catalog_diag_public_spec(spec):
    if not spec:
        return None
    allowed = (
        "composition", "primary_metal", "fineness_per_mille", "weight_g",
        "diameter_mm", "fine_metal_g", "confidence", "spec_source",
        "spec_source_urls", "spec_evidence_counts", "spec_evidence_by_field",
        "listings_checked",
    )
    return {k: spec.get(k) for k in allowed if k in spec}

@app.get("/api/admin/catalog-builder-test")
def catalog_builder_test():
    # Disabled unless an explicit secret exists in Render.
    if not CATALOG_DIAGNOSTIC_KEY:
        return jsonify({
            "ok": False,
            "error": "catalog_diagnostic_disabled",
            "message": "Set CATALOG_DIAGNOSTIC_KEY in the Render environment first."
        }), 503
    ...
    started = time.time()
    try:
        spec = mashops_spec_fallback(coin, query)
        ...
        return jsonify({
            ...
            "consensus_found": bool(spec),
            "spec": _catalog_diag_public_spec(spec),
            "message": "Consensus found; database unchanged." if spec else
                       "No accepted consensus; database unchanged.",
        })
    except Exception as e:
        app.logger.exception("catalog-builder diagnostic failed")
        return jsonify({
            "ok": False,
            "mode": "DRY_RUN",
            "database_writes": False,
