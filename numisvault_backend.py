...
# ============================================================
# TEMPORARY CATALOGUE CONSENSUS DIAGNOSTIC V5 — DRY RUN ONLY
# Added 2026-08-22.
#
# V5 changes:
# - Cross-year canonical weight requires >=2 years AND >=3 unique dealers.
# - Early-stop only after that stricter threshold is reached.
...
def _catalog_series_weight_fallback(coin, target_canonical, time_budget_seconds=12.0, max_years=4):
    ...
    for yy in _catalog_series_year_candidates(coin):
        ...
        # EARLY STOP: only after a weight is supported by >=2 years,
        # >=3 dealer-year observations, and >=3 UNIQUE dealers.
        candidates=[]
        for weight,rows2 in weight_groups.items():
            ...
            if len(years)>=2 and len(rr)>=3 and len(dealers)>=3:
                candidates.append({
                    "weight_g":weight,
        ...
        if candidates:
            ...
            return {
                ...
                "years_checked":years_checked,
                "elapsed_seconds":round(time.time()-started,3),
                "method":"MA_SHOPS_SAME_SERIES_CROSS_YEAR_3_DEALER_EARLY_STOP",
                "year_reports":year_reports,
            }

    return {
        "accepted":False,
        "reason":"no_cross_year_weight_met_3_unique_dealer_threshold",
        "years_checked":years_checked,
        "elapsed_seconds":round(time.time()-started,3),
        "year_reports":year_reports,
    }
    ...
    try:
        ...
        if (
            ...
            fallback=_catalog_series_weight_fallback(
                coin,
                canonical,
                time_budget_seconds=min(12.0,max(2.0,remaining-2.0)),
                max_years=4,
            )
            if fallback.get("accepted"):
                ...
                catalogue["notes"].append(
                    "Canonical weight filled from same-series cross-year MA-Shops evidence: >=2 years and >=3 unique dealers."
                )
        ...
        return jsonify({
            ...
            "elapsed_seconds":round(time.time()-started,3),
            "catalogue":catalogue,
            "message":"Catalogue V5 evaluated; database unchanged.",
        })
    except Exception as e:
        app.logger.exception("catalog-builder diagnostic V5 failed")
        return jsonify({
            "ok":False,
            "mode":"DRY_RUN_V5",
            "database_writes":False,
