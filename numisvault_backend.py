...
def extract_mashops_explicit_specs(html_text, source_url=None):
    """Extract MA-Shops physical specs using explicit labels only.

    MA-Shops often renders each specification in a separate sibling element.
    Therefore extraction is done from normalized visible page text, but every
    numeric value MUST still be directly attached to its own label. Bare
    numbers/catalogue numbers can never become weight, diameter or fineness.
    """
    if not html_text:
        return []
    soup=BeautifulSoup(html_text,"html.parser")
    ...
    # Prefer the main item/content area when available; otherwise use body.
    root=(soup.select_one("main") or soup.select_one("#content") or
          soup.select_one(".content") or soup.body or soup)
    text=smart_join(root.stripped_strings)
    if not text:
        return []

    rec={"source_url":source_url,"evidence":""}
    snippets=[]

    def capture(pattern, flags=re.I):
        m=re.search(pattern,text,flags)
        if m:
            snippets.append(m.group(0)[:180])
        return m

    wm=capture(
        r"\b(?:weight|gewicht|poids|peso)\s*[:\-]?\s*"
        r"([0-9]+(?:[.,][0-9]+)?)\s*(?:g|gr|gram|grams|gramm)\b")
    if wm:
        v=num(wm.group(1))
        if v is not None and 0.05<=v<=2000:
            rec["weight_g"]=float(v)

    dm=capture(
        r"\b(?:diameter|durchmesser|diam[eè]tre|diametro)\s*[:\-]?\s*"
        r"([0-9]+(?:[.,][0-9]+)?)\s*mm\b")
    if not dm:
        dm=capture(r"(?:Ø|ø)\s*([0-9]+(?:[.,][0-9]+)?)\s*mm\b")
    if dm:
        v=num(dm.group(1))
        if v is not None and 3<=v<=200:
            rec["diameter_mm"]=float(v)
    ...
    metal_aliases={
        ...
        "palladium":"Palladium","copper":"Copper","kupfer":"Copper","cuivre":"Copper","cobre":"Copper",
        "bronze":"Bronze","brass":"Brass","messing":"Brass","nickel":"Nickel",
        "steel":"Steel","stahl":"Steel","aluminium":"Aluminium","aluminum":"Aluminium"
    }

    # Material is captured only immediately after an explicit material label,
    # and stops at the next known catalogue/spec label.
    cm=capture(
        r"\b(?:material|composition|metal|legierung|materiale|m[ée]tal)\s*[:\-]?\s*"
        r"(.{1,90}?)(?=\s+(?:weight|gewicht|poids|peso|fineness|feinheit|titre|ley|"
        r"diameter|durchmesser|diam[eè]tre|diametro|catalog|catalogue|katalog|"
        r"mintage|mint|edge|ruler|subject|grade|image)\b|$)")
    ...
    if comp:
        ...
        for raw,canon in metal_aliases.items():
            if re.search(rf"\b{re.escape(raw)}\b",comp,re.I):
                rec["primary_metal"]=canon
                break

    # Fineness MUST be label-bound. This is the protection against the old .188 bug.
    fm=capture(
        r"\b(?:fineness|feinheit|titre|ley)\s*[:\-]?\s*"
        r"(?:0[.,])?([0-9]{3})(?:\s*(?:‰|/ ?1000))?")
    if fm:
        v=int(fm.group(1))
        if 100<=v<=999:
            rec["fineness_per_mille"]=v
    elif comp:
        # Also accept an explicit decimal fineness inside Material itself:
        # "Silver (.900)" / "Silver 0.900".
        fm2=re.search(r"(?:0[.,]|\.)([0-9]{3})\b",comp)
        if fm2:
            v=int(fm2.group(1))
            if 100<=v<=999:
                rec["fineness_per_mille"]=v
                snippets.append("Material: "+comp)

    rec["evidence"]=" | ".join(dict.fromkeys(snippets))

    # One page is useful if it contains at least two explicit facts.
    facts=sum(k in rec for k in ("primary_metal","fineness_per_mille","weight_g","diameter_mm"))
    return [rec] if facts>=2 else []
...
def mashops_spec_fallback(coin, raw_query=""):
    ...
    # If consensus is unavailable because other correct listings omit specs,
    # accept ONE exact page only when it explicitly states the complete
    # precious-metal core: material + fineness + weight. This is still
    # label-bound and cannot recreate the old bare-number/.188 failure.
    complete_single=None
    for er in evidence:
        if (er.get("primary_metal") and er.get("fineness_per_mille") is not None
                and er.get("weight_g") is not None):
            complete_single=er
            break
    ...
    if not accepted and complete_single:
        accepted["primary_metal"]=complete_single["primary_metal"]
        accepted["fineness_per_mille"]=int(complete_single["fineness_per_mille"])
        accepted["weight_g"]=round(float(complete_single["weight_g"]),4)
        if complete_single.get("diameter_mm") is not None:
            accepted["diameter_mm"]=round(float(complete_single["diameter_mm"]),3)
        groups={
            "primary_metal":[complete_single],
            "fineness_per_mille":[complete_single],
            "weight_g":[complete_single],
            "diameter_mm":[complete_single] if complete_single.get("diameter_mm") is not None else []
        }

    # Precious-metal fineness without a validated weight is suppressed.
    ...
    result={
        **accepted,
        ...
        "match_class":("MA_SHOPS_EXACT_LABELLED_ITEM" if complete_single and max(counts.values() or [0])<2 else "MA_SHOPS_MULTI_LISTING_CONSENSUS"),
        "confidence":(0.88 if complete_single and max(counts.values() or [0])<2 else 0.92),
        "spec_source":("MA-Shops exact labelled item specification" if complete_single and max(counts.values() or [0])<2 else "MA-Shops multi-listing consensus"),
...
def inspect_mashops_item_asset(url, timeout=7):
    ...
    try:
        ...
        if _ma_shops_banknote_evidence(combined):
...
def validate_ambiguous_mashops_assets(offers, payload, max_checks=18):
    ...
    filtered=[o for o in offers if o.get("_asset_page_validation")!="BANKNOTE"]
...
def coin_search():
    ...
    valid,asset_detail_checked,asset_detail_rejected=validate_ambiguous_mashops_assets(valid,payload,max_checks=18)
    if asset_detail_rejected:
        rejected["asset"]+=asset_detail_rejected
    print(f"[coin-search] asset-detail checked={asset_detail_checked} rejected_banknotes={asset_detail_rejected} valid_after={len(valid)}",flush=True)
