...
def _cluster_numeric_evidence(items, abs_tol, rel_tol=0.0):
    """Return the strongest numeric consensus cluster from independent listings."""
    best=[]
    for anchor in items:
...
def mashops_spec_consensus(coin, raw_query="", max_item_pages=15, min_agree=2):
...
def _merge_spec_records(primary, secondary):
    """Fill missing fields only; never let a weaker source overwrite source #1."""
    ...
    # Preserve complete provenance rather than pretending merged facts came from one page.
    chain=list(out.get("source_chain") or [])
    for rec in (primary,secondary):
        src={
            "provider":rec.get("data_provider") or rec.get("provider"),
    ...
    out["spec_source"]=" + ".join(x for x in [
        primary.get("spec_source"), secondary.get("spec_source")] if x)
    out["data_provider"]="CoinBids resolved specifications"
    out["match_class"]="COINBIDS_SOURCE_FUSION"
    out["confidence"]=max(float(primary.get("confidence") or 0),float(secondary.get("confidence") or 0))
    return out
...
def coin_lookup():
    ...
    baseline=_merge_spec_records(source1,source2)
    ...
    if _spec_has_any(baseline):
        print(f"[coin-lookup] baseline source1={source1_provider} ma_consensus={bool(source2)} "
              ...
              f"diameter={baseline.get('diameter_mm')}",flush=True)
