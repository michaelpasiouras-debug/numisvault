...
# ============================================================
# TEMPORARY CATALOGUE CONSENSUS DIAGNOSTIC V3 — DRY RUN ONLY
# Added 2026-08-22.
#
# V3 adds a SERIES-LEVEL MA-SHOPS fallback for missing canonical
# weight. It remains MA-Shops-only and performs NO database writes.
#
# Safety:
# - Existing production routes are untouched.
# - No INSERT / UPDATE / DELETE.
# - Current-year evidence is preferred.
# - Cross-year fallback is used only for the SAME explicit
#   country + denomination + variant/type.
# - The fallback must agree on metal + fineness and must find the
#   same repeated weight across >=2 different years.
# ============================================================
...
def _catalog_series_year_candidates(coin, radius=4):
    """Nearby same-series years; target year excluded."""
...
def _catalog_series_weight_fallback(coin, target_canonical, max_years=6):
    """Find a canonical nominal weight from SAME-series MA-Shops evidence.

    Conservative acceptance:
    - explicit variant/type is mandatory
    - target already has verified metal + fineness
    - same country + denomination + variant across all probe years
    - candidate weight must appear in >=2 DIFFERENT years
    - >=3 total independent dealer-year observations support that weight
    - each supporting year's accepted metal/fineness must match target
    """
    variant=str(coin.get("variant") or "").strip()
    if not variant:
    ...
    for yy in _catalog_series_year_candidates(coin)[:max_years]:
        ...
        # We only accept evidence from a year whose type's metal/fineness match target.
        if c.get("primary_metal")!=target_metal or c.get("fineness_per_mille")!=target_fine:
            year_reports.append({
                "year":yy,"status":"SKIPPED_COMPOSITION_MISMATCH",
                "canonical":c
            })
            continue
    ...
    candidates=[]
    for weight,rows in weight_groups.items():
        # Deduplicate by (year,dealer)
        uniq={}
        for r in rows: uniq[(r["year"],r["dealer"])]=r
        rows=list(uniq.values())
        years=sorted(set(r["year"] for r in rows))
        dealers=sorted(set(r["dealer"] for r in rows))
        if len(years)>=2 and len(rows)>=3:
            candidates.append({
                "weight_g":weight,
    ...
    # Reject ties between competing weights with equal support.
    if len(candidates)>1:
        top=(best["observation_count"],len(best["supporting_years"]),best["dealer_count"])
        second=(candidates[1]["observation_count"],len(candidates[1]["supporting_years"]),candidates[1]["dealer_count"])
        if top==second and candidates[1]["weight_g"]!=best["weight_g"]:
            return {
                "accepted":False,
                "reason":"ambiguous_cross_year_weight_tie",
    ...
    return {
        ...
        "years_checked":years_checked,
        "method":"MA_SHOPS_SAME_SERIES_CROSS_YEAR_CONSENSUS",
    }
...
def catalog_builder_test():
    ...
    try:
        ...
        if "weight_g" not in canonical and canonical.get("primary_metal") and canonical.get("fineness_per_mille") is not None:
            fallback=_catalog_series_weight_fallback(coin,canonical,max_years=6)
            if fallback.get("accepted"):
                ...
                catalogue["catalogue_status"]=(
                    "VERIFIED_COMPLETE"
                    if "diameter_mm" in canonical
                    else "VERIFIED_METAL_SPECS"
                )
                catalogue["notes"].append(
                    "Canonical weight filled from same-series cross-year MA-Shops consensus; "
                    "current-year specimen weights remain observations only."
                )
        ...
        return jsonify({
            ...
            "catalogue":catalogue,
            "message":"Catalogue V3 evaluated; database unchanged.",
        })
    except Exception as e:
        app.logger.exception("catalog-builder diagnostic V3 failed")
        return jsonify({
            "ok":False,
            "mode":"DRY_RUN_V3",
            "database_writes":False,
