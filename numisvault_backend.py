...
def coin_lookup():
    ...
    # Zero-quota path: satisfy known physical specifications locally first —
    # PostgreSQL (the richer, ~591-record database) checked before the small
    ...
    if not local_match:
        ...
        provider="local"
    if local_match:
        return jsonify({"match":local_match,"provider":provider,"note":"Physical specifications resolved locally; no Numista API request was used."})

    # Live commercial-catalogue fallback: MA-Shops often exposes explicit
    # weight + precious-metal fineness on the exact item page. Reuse our
    # existing strict identity validation before accepting those fields.
    # This avoids guessing and can also save Numista quota.
...
def coin_search():
    ...
    direct_enrich_cap=max(4,min(int(payload.get("direct_enrich_cap") or 10),14))
    ...
    # Normalize item prices first without making network requests.
    for o in valid:
        finalize(o)

    direct_checked=0
    if include_shipping:
        for idx,o in enumerate(by_price):
            if direct_checked>=direct_enrich_cap:
                break

            confirmed_now=sorted(
                [x for x in valid if x.get("total") is not None],
                key=lambda x:x["total"]
            )
            if len(confirmed_now)>=2:
                second_best=float(confirmed_now[1]["total"])
                item_floor=o.get("price")
                if item_floor is not None and float(item_floor)>second_best:
                    print(
                        f"[coin-search] direct enrichment early-stop idx={idx} "
                        f"item_floor={item_floor} > second_best_total={second_best}",
                        flush=True
                    )
                    break

            if not o.get("detail_page_checked") or o.get("shipping_status") in ("unknown","unverified",None):
                enrich_offer_from_item_page(o,ship_to_country,use_geo_proxy=False)
                direct_checked+=1
                finalize(o)

    print(
        f"[coin-search] valid={len(valid)} direct_enriched={direct_checked}/"
        f"{direct_enrich_cap} ship_to={ship_to_country} "
    ...
    # Geo-verify only plausible top-2 contenders. If an unknown-shipping
    # candidate's item price is already above the current #2 confirmed total,
    # it cannot mathematically enter the top two.
    geo_checked=0
    geo_cap=max(2,min(int(payload.get("geo_enrich_cap") or 6),8))
    if include_shipping and SCRAPINGBEE_API_KEY:
        ...
        if len(confirmed)>=2:
            confirmed=sorted(confirmed,key=lambda o:o["total"])
            ...
            for o in unknown_by_price:
                ...
                if ip<=second_best_total+margin:
                    ...
                    break
                if len(contenders)>=geo_cap:break
        else:
            contenders=unknown_by_price[:geo_cap]
    ...
    # Final safety pass is limited to the 1-2 offers actually displayed.
    if include_shipping and SCRAPINGBEE_API_KEY:
        for o in top:
            if o.get("shipping_status")!="known_target":
                enrich_offer_from_item_page(o,ship_to_country,use_geo_proxy=True)
    ...
    print(
        f"[coin-search] enrichment summary direct={direct_checked} "
        f"geo={geo_checked} displayed={len(top)}",
        flush=True
    )
