...
new='''        if _raw_for_target:
            try:
                target_identity=resolve_coin_identity(_raw_for_target).get("best")
                # IMPORTANT: the frontend may send only coin.raw.  Hard filtering
                # must nevertheless use the canonical identity resolved above;
                # otherwise year/denomination/country filters silently disappear.
                if target_identity:
...
'''
if old not in s:
    raise SystemExit('resolver enrichment block not found')
s=s.replace(old,new,1)

old2='''    top=valid[:max(1,min(int(payload.get("limit") or 2),2))]
'''
new2='''    # UI still defaults to two cards, but QA/audit callers may explicitly
    # request the complete validated set so the claimed cheapest result can be
    # independently proved against every validated offer.
...
'''
if old2 not in s:
    raise SystemExit('top limit block not found')
...
'''
if old3 not in s:
    raise SystemExit('result tail block not found')
